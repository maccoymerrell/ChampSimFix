...
#ifndef NMFC_HOST_CORE_H
#define NMFC_HOST_CORE_H
...
// NMFC: the function network this core dispatches onto
...
class NMFC_CacheBus
{
  ...
  friend class NMFC_HOST_CORE;

public:
  explicit NMFC_CacheBus(champsim::modules::channel_module* ll) : lower_level(ll) {}
  bool issue_read(request_type packet);
  bool issue_write(request_type packet);
};

struct NMFC_LSQ_ENTRY : champsim::program_ordered<NMFC_LSQ_ENTRY> {
  ...
  std::vector<std::reference_wrapper<std::optional<NMFC_LSQ_ENTRY>>> lq_depend_on_me{};
  ...
  NMFC_LSQ_ENTRY(champsim::address addr, champsim::program_ordered<NMFC_LSQ_ENTRY>::id_type id, champsim::address ip, champsim::origin origin);
  void finish(ooo_model_instr& rob_entry) const;
};

// cpu
// NMFC: offload_sink is how a completed invocation gets back here. It is a
// plain abstract base, so it costs no registry slot.
class NMFC_HOST_CORE : public champsim::modules::core_module, public nmfc::offload_sink
{
public:
...
private:
  ...
  std::vector<std::optional<NMFC_LSQ_ENTRY>> LQ;
  ...
  champsim::ring_buffer<NMFC_LSQ_ENTRY> SQ;

public:
  // Inspection
  const champsim::ring_buffer<ooo_model_instr>& rob() const { return ROB; }
  const std::vector<std::optional<NMFC_LSQ_ENTRY>>& lq() const { return LQ; }
  const champsim::ring_buffer<NMFC_LSQ_ENTRY>& sq() const { return SQ; }
...
private:
  ...
  struct rob_mem_handle {
    ...
    bounded_array<NMFC_LSQ_ENTRY*, 4> sq_entries{};
  };
  ...
  std::unordered_map<uint64_t, std::vector<NMFC_LSQ_ENTRY*>> sq_store_index_;
  ...
  //
  // Parallel to the LSQ, not a reuse of it: offload concurrency is ftu_size,
  // its own knob, rather than an accident of lq_size. An entry is allocated
...
public:
  ...
  void do_sq_forward_to_lq(NMFC_LSQ_ENTRY& sq_entry, NMFC_LSQ_ENTRY& lq_entry);

  void do_finish_store(const NMFC_LSQ_ENTRY& sq_entry);
  bool do_complete_store(const NMFC_LSQ_ENTRY& sq_entry);
  bool execute_load(const NMFC_LSQ_ENTRY& lq_entry);
  ...
  virtual ~NMFC_HOST_CORE() noexcept = default;

  explicit NMFC_HOST_CORE(champsim::modules::ModuleBuilder builder)
        ...
        L1D_bus(builder.get_parameter<champsim::modules::channel_module*>("data_queues")), l1i(builder.get_parameter<champsim::modules::cache_module*>("l1i"))
  {
    ...
    // NMFC: the tracking unit, and our identity on the function network. The
    // fabric is declared before us in the configuration, so it already exists
    // and we register rather than being registered.
