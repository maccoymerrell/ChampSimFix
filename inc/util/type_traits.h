...
auto numeric_any_cast(const std::any& val, T& out) -> std::enable_if_t<std::is_arithmetic_v<T>, bool>
{
  ...
  if (auto* p = std::any_cast<unsigned>(&val)) {
    out = static_cast<T>(*p);
    return true;
  }
  if (auto* p = std::any_cast<long>(&val)) {
    out = static_cast<T>(*p);
    return true;
  }
  if (auto* p = std::any_cast<unsigned long>(&val)) {
    out = static_cast<T>(*p);
    return true;
  }
  if (auto* p = std::any_cast<long long>(&val)) {
    out = static_cast<T>(*p);
    return true;
  }
  if (auto* p = std::any_cast<unsigned long long>(&val)) {
    out = static_cast<T>(*p);
    return true;
  }
  if (auto* p = std::any_cast<uint8_t>(&val)) {
    out = static_cast<T>(*p);
    return true;
  }
  if (auto* p = std::any_cast<uint16_t>(&val)) {
    out = static_cast<T>(*p);
    return true;
  }
  if (auto* p = std::any_cast<uint32_t>(&val)) {
    out = static_cast<T>(*p);
    return true;
  }
  if (auto* p = std::any_cast<uint64_t>(&val)) {
    out = static_cast<T>(*p);
    return true;
  }
  if (auto* p = std::any_cast<int8_t>(&val)) {
    out = static_cast<T>(*p);
    return true;
  }
  if (auto* p = std::any_cast<int16_t>(&val)) {
    out = static_cast<T>(*p);
    return true;
  }
  if (auto* p = std::any_cast<int32_t>(&val)) {
    out = static_cast<T>(*p);
    return true;
  }
  if (auto* p = std::any_cast<int64_t>(&val)) {
    out = static_cast<T>(*p);
    return true;
  }
