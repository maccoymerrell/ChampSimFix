...
struct prefetcher : public bound_to<CACHE> {
  ...
  bool prefetch_line(champsim::address pf_addr, bool fill_this_level, uint32_t pf_cpu, champsim::address pf_ip, uint32_t prefetch_metadata, bool skip_tag_check = false, bool return_hit_status = false, int pf_distance = -1) const;
  void drop_prefetch_access(champsim::address pf_addr) const;
