...
public:
  ...
  // SHARED residency/prefetch filter: when set, probe() consults this instead of the internal filt_,
  // so the branch graph and the data prefetcher use ONE residency view (neither re-issues a block the
  // other already holds / has in flight). Used by the holistic I+D harness. (mark() still writes filt_
  // harmlessly; the shared view is maintained by whoever installs into the shared L2.)
  void set_shared_probe(std::function<bool(uint64_t)> f) { shared_probe_ = std::move(f); }
...
private:
  ...
  bool probe(uint64_t blk) const { if (shared_probe_) return shared_probe_(blk); return filt_[static_cast<std::size_t>(hash(blk) & fmask_)] == ftag(blk); }
  ...
  std::function<bool(uint64_t)> shared_probe_; // when set, the shared residency view (else internal filt_)
  int64_t dlim_ = 128;
  uint64_t last_ip_ = 0;
