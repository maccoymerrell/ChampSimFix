...
struct params {
  ...
  // --- Directionality ---
  bool do_negative = true;  // PROMOTED: gated backward scan captures backward-strided pointer-chase (mcf +47% cov)
  ...
  bool neg_online_train = true; // PROMOTED: train backward patterns per-access (else the negative tables stay cold)
  bool neg_train_gated = true;  // PROMOTED: train backward only when the direction is negative (avoids 0-key pollution on forward streams)
  bool neg_dir_pc = true;       // PROMOTED: direction signal = per-IP ip_dir_ (PC-correlated); cleaner than spatial momentum
  ...
  bool bwd_useful_gate = true;       // PROMOTED: self-throttle backward on its own sampled per-IP usefulness (kills forward-stream residue)
