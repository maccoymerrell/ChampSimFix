...
class ampm : public champsim::modules::prefetcher
{
  ...
  public:
    class AMPM_Module {
      ...
      void do_prefetch(CACHE* intern, champsim::address addr, champsim::address ip, uint32_t cpu, uint8_t cache_hit, bool useful_prefetch, access_type type, uint32_t metadata_in, int degree, bool two_level, bool report);
    };
  ...
  std::vector<AMPM_Module> modules;
