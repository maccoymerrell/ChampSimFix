...
void ampm::AMPM_Module::do_prefetch(CACHE* intern, champsim::address addr, champsim::address ip, uint32_t cpu, uint8_t cache_hit, bool useful_prefetch, access_type type, uint32_t metadata_in, int degree, bool two_level, bool report) {
    ...
    for (int i = 1, prefetches_issued = 0; prefetches_issued < degree; i++) {
      ...
      if (check_pagemap(champsim::address{neg_step_addr},false) && check_pagemap(champsim::address{neg_2step_addr},false) && !check_pagemap(champsim::address{pos_step_addr},false) && !check_pagemap(champsim::address{pos_step_addr},true)) {
        ...
        if (block_addr != champsim::block_number{pos_step_addr}) {
          ...
          if (bool prefetch_success = intern->prefetch_line(pf_addr, two_level, cpu, ip, metadata_in,false,report); prefetch_success) {
