#ifndef PREFETCHER_SPPAM_VIRT_H
#define PREFETCHER_SPPAM_VIRT_H
...
class sppam_virt : public champsim::modules::prefetcher
{
  public:
    struct Sppam_virt_Module {
      CACHE* intern_;
      static constexpr std::size_t REGION_SETS = 64;
      static constexpr std::size_t REGION_WAYS = 12;
      ...
      static constexpr unsigned int SPPAM_VIRT_PAGE_BITS = 12;
      ...
      static constexpr uint64_t SCRAPE_IDLE_TIME = 100;
      ...
      static constexpr uint64_t LOOKAHEAD_DEPTH = 16;
      ...
      static constexpr uint64_t SCAN_DISTANCE_FORWARD = PATTERN_SIZE;
      static constexpr bool SCAN_BACKWARD = true;
      static constexpr uint64_t SCAN_DISTANCE_BACKWARD = PATTERN_SIZE;
      ...
      struct page_extent : champsim::dynamic_extent {
        page_extent() : dynamic_extent(champsim::data::bits{64}, champsim::data::bits{SPPAM_VIRT_PAGE_BITS}) {}
      };
      using page = champsim::address_slice<page_extent>;

      struct block_in_page_extent : champsim::dynamic_extent {
        block_in_page_extent() : dynamic_extent(champsim::data::bits{SPPAM_VIRT_PAGE_BITS}, champsim::data::bits{LOG2_BLOCK_SIZE}) {}
      };
      ...
      struct region_type {
        ...
        explicit region_type(page allocate_vpn, uint64_t last_access_time_)
          : vpn(allocate_vpn), next_vpn(page{}), prev_vpn(page{}), access_map((1 << SPPAM_VIRT_PAGE_BITS) / BLOCK_SIZE), prefetch_map((1 << SPPAM_VIRT_PAGE_BITS) / BLOCK_SIZE), last_access_time(last_access_time_), pf_trigger(((1 << SPPAM_VIRT_PAGE_BITS) / BLOCK_SIZE))
        {
        }
      };


      struct sppam_virt_indexer {
        auto operator()(const region_type& entry) const { return entry.vpn; }
      };
      champsim::msl::lru_table<region_type,sppam_virt_indexer,sppam_virt_indexer> regions{REGION_SETS,REGION_WAYS};
      ...
      struct pattern_type {
        ...
        sppam_virt_util::msl::conf_table<uint64_t,pattern_conf_indexer,pattern_conf_indexer> prediction_table{PATTERN_CONF_SETS,PATTERN_CONF_SETS_WAYS};
      ...
      void add_to_pagemap(champsim::address addr, bool prefetch, champsim::address ip = champsim::address{}, uint64_t pf_trigger = 0);
