...
template <typename T>
auto sppam_virt::Sppam_virt_Module::page_and_offset(T addr) -> std::pair<page, block_in_page>
{
  return std::pair{page{addr}, block_in_page{addr}};
}

std::pair<uint64_t,uint64_t> sppam_virt::Sppam_virt_Module::get_patterns(page pn, block_in_page page_offset) {
...
bool sppam_virt::Sppam_virt_Module::check_pagemap(champsim::address addr, bool prefetch)
{
...
std::pair<uint64_t,bool> sppam_virt::Sppam_virt_Module::get_prefetch_pattern(uint64_t pattern, int order, bool negative) {
...
void sppam_virt::Sppam_virt_Module::increment_access_pattern(uint64_t pattern, uint64_t prediction, int order, bool negative) {
  auto temp_pattern = pattern_type{pattern & ((1 << (PATTERN_SIZE >> order)) - 1)};
  auto demand_pattern = negative && SEPARATE_NEGATIVE_TABLES ?  negative_pattern_tables.at(order).check_hit(temp_pattern) : pattern_tables.at(order).check_hit(temp_pattern);
...
void sppam_virt::Sppam_virt_Module::initialize(CACHE* cache) {
  intern_ = cache;
  ...
  fmt::print("[{}] SPPAM_VIRT Initialized. Total State: {}\n",intern_->NAME,champsim::data::kibibytes{total_bytes});
}



void sppam_virt::Sppam_virt_Module::add_to_pagemap(champsim::address addr, bool prefetch, champsim::address ip, uint64_t pf_trigger) {
  ...
  if(demand_region.has_value()) {
    ...
    if(prefetch) {
      ...
      demand_region->prev_vpn = demand_region->vpn - 1;
      demand_region->next_vpn = demand_region->vpn + 1;
    }
    ...
    if(prefetch) {
      ...
      temp_region.prev_vpn = temp_region.vpn - 1;
      temp_region.next_vpn = temp_region.vpn + 1;
    }
...
void sppam_virt::Sppam_virt_Module::remove_from_pagemap(champsim::address addr, bool prefetch) {
  auto [current_pn, page_offset] = page_and_offset(addr);
...
uint32_t sppam_virt::prefetcher_cache_operate(champsim::address addr, champsim::address ip, bool cache_hit, bool useful_prefetch, access_type type, uint32_t metadata_in)
{
  ...
  if((!cache_hit || !engine.ACCESS_MAP_MISS_ONLY))
    engine.add_to_pagemap(addr,false,ip,0);
...
uint32_t sppam_virt::prefetcher_cache_fill(champsim::address addr, long set, long way, bool prefetch, champsim::address evicted_addr, uint32_t metadata_in)
{
...
void sppam_virt::Sppam_virt_Module::track_llc_missrate(uint32_t hit) {
  assert(!TRACK_LLC_MISSRATE);
}

void sppam_virt::Sppam_virt_Module::scrape_region(champsim::address addr) {
...
int sppam_virt::Sppam_virt_Module::get_momentum(champsim::address addr) {
  auto [pn, page_offset] = page_and_offset(addr);
  auto region = regions.check_hit(region_type{pn,intern_->current_cycle()});
  if(region.has_value()) {
...
void sppam_virt::Sppam_virt_Module::do_prefetch(champsim::address addr, champsim::address ip, uint8_t cache_hit, bool useful_prefetch, access_type type, uint32_t metadata_in) {
  champsim::block_number block_addr{addr};
  if(type != access_type::PREFETCH) {
    metadata_in = (ip.to<uint64_t>() & BERTI_IP_MASK) << BERTI_IP_ENCODING_OFFSET;
    //fmt::print("[SPPAM_VIRT] Sent IP of: {}\n",ip.to<uint64_t>() & BERTI_IP_MASK);
  }
              ...
              if(pos_prefetch_pattern & (1 << (i - 1 - j))) {
                ...
                if(!check_pagemap(champsim::address{pos_step_addr},true)) {
                  if(true) { //filter LLC prefetches
                    if(bool prefetch_success = intern_->prefetch_line(pos_step_addr, pf_issued < free_space, lookahead_offset < 50 ? metadata_in + 256 : 0); prefetch_success) {
                      if((!in_warmup) && DO_DEBUG)
            ...
            for(int j = 0; j < i; j++) {
              ...
              if(neg_prefetch_pattern & (1 << (i - 1 - j))) {
                ...
                if(!check_pagemap(champsim::address{neg_step_addr},true)) {
                  if(true) {
                    if(bool prefetch_success = intern_->prefetch_line(neg_step_addr, pf_issued < free_space,lookahead_offset < 50 ? metadata_in : 0); prefetch_success) {
...
void sppam_virt::Sppam_virt_Module::print_patterns() {
...
uint64_t sppam_virt::Sppam_virt_Module::get_state_bits() {
  //Assuming 48-bit max address
  //calculate total bits needed for Sppam_virt_Module
  uint64_t total_bits = 0;

  //start with region table
  uint64_t region_table_bits = (48 - champsim::lg2(SPPAM_VIRT_PAGE_BITS)) * 1; //page nums
  region_table_bits += ((1 << SPPAM_VIRT_PAGE_BITS) / BLOCK_SIZE)*2; //bitmaps
  region_table_bits += SCRAPE_ON_IDLE ? champsim::lg2(SCRAPE_IDLE_TIME) : 0; //scrape count
  region_table_bits += REGION_PATTERN_TAG ? (((1 << SPPAM_VIRT_PAGE_BITS) / BLOCK_SIZE) * PATTERN_SIZE) : 0;
  //region_table_bits += 4 //momentum (DISABLED)
  //region_table_bits += champsim::lg2(((1 << SPPAM_VIRT_PAGE_BITS) / BLOCK_SIZE)); //last block
  ...
  //total state tally
  total_bits += region_table_bits + pattern_table_bits + misc_state; //+ cpt bits


  return total_bits;
}

bool sppam_virt::Sppam_virt_Module::use_pattern_confidence() {
  //static 
...
void sppam_virt::Sppam_virt_Module::modify_pattern_usefulness(champsim::address addr, bool useful) {
  auto [pn, page_offset] = page_and_offset(addr);
  auto region = regions.check_hit(region_type{pn,intern_->current_cycle()});
  if(!region.has_value()) {
    if(!useful)
...
void sppam_virt::prefetcher_cycle_operate() {
  if(intern_->current_cycle() % 1000000 == 0) {
    fmt::print("[{}] SPPAM_VIRT\n",intern_->NAME);
  ...
  if(!intern_->warmup && engine.in_warmup) {
    fmt::print("[{}] SPPAM_VIRT Resetting internal stats\n",intern_->NAME);
...
int sppam_virt::Sppam_virt_Module::set_prefetch_degree(uint64_t pattern, int order, bool negative, int prev_usefulness = 15) {
...
void sppam_virt::Sppam_virt_Module::decrease_usefulness_counter() {
  /*
...
void sppam_virt::Sppam_virt_Module::increase_usefulness_counter() {
  /*
...
int sppam_virt::Sppam_virt_Module::do_4_bit_mult(int op1, int op2) {
