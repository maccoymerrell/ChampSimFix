...
// ChampSim's modular build no longer exposes the global LOG2_BLOCK_SIZE macro
// (modules read block_size from the config). Bingo uses it in compile-time
// contexts, so define it here for the standard 64-byte cache line used by all
// DPC4 traces.
...
class bingo : public champsim::modules::prefetcher{
  ...
  private:
    ...
    public:
        ...
        explicit bingo(champsim::modules::ModuleBuilder builder);

        // The parent cache, captured at construction. intern_ is private in the
        // runtime base class; PrefetchStreamer::prefetch reads queue/MSHR state
        // through this pointer, so it is public.
