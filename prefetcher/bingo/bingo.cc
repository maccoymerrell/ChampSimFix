...
champsim::modules::prefetcher::register_module<bingo> bingo_register("bingo");

bingo::bingo(champsim::modules::ModuleBuilder builder) : cache_(builder.get_parent<champsim::modules::cache_module>()) {}
...
int PrefetchStreamer::prefetch(bingo *b, uint64_t block_address) {
    if (this->debug_level >= 2) {
        std::cerr << "PrefetchStreamer::prefetch(cache=" << b->cache_->NAME << ", block_address=0x" << std::hex << block_address
                << ")" << std::dec << std::endl;
        std::cerr << "[PrefetchStreamer::prefetch] " << b->cache_->get_pq_occupancy().back() << "/" << b->cache_->get_pq_size().back()
                << " PQ entries occupied." << std::dec << std::endl;
        std::cerr << "[PrefetchStreamer::prefetch] " << b->cache_->get_mshr_occupancy() << "/" << b->cache_->get_mshr_size()
    ...
    for (int d = 1; d < this->pattern_len; d += 1) {
        ...
        for (int sgn = +1; sgn >= -1; sgn -= 2) {
            ...
            if (0 <= pf_offset && pf_offset < this->pattern_len && pattern[pf_offset] > 0) {
                ...
                if (b->cache_->get_pq_occupancy().back() + b->cache_->get_mshr_occupancy() < b->cache_->get_mshr_size() - 1 &&
                    b->cache_->get_pq_occupancy().back() < b->cache_->get_pq_size().back()) {
...
int bingo::prefetch(uint64_t block_number) {
    if (this->debug_level >= 2)
        std::cerr << "Bingo::prefetch(cache=" << cache_->NAME << ", block_number=" << std::hex << block_number << ")" << std::dec
                << std::endl;
...
uint32_t bingo::prefetcher_cache_operate(champsim::address addr, champsim::address ip, bool cache_hit, bool useful_prefetch, access_type type, uint32_t metadata_in)
{
    if (DEBUG_LEVEL >= 2) {
        ...
        std::cerr << "[CACHE::l1d_prefetcher_operate] CACHE{NAME=" << cache_->NAME << "}" << std::dec
             << std::endl;
    }
    ...
    this->access(block_number.to<uint64_t>(), ip.to<uint64_t>());

    /* issue prefetches */
    this->prefetch(block_number.to<uint64_t>());
...
uint32_t bingo::prefetcher_cache_fill(champsim::address addr, long set, long way, bool prefetch, champsim::address evicted_addr, uint32_t metadata_in) {
    ...
    for (bingo* p : prefetchers)
        p->eviction(evicted_block_number.to<uint64_t>());
