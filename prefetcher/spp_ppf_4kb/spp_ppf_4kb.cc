...
void spp_ppf_4kb::PPF_Module::init(CACHE* cache)
{
    cache_ = cache;
	for(int a = 0; a < 30; a++)
		depth_track[a] = 0;
...
void spp_ppf_4kb::PPF_Module::handle_fill(champsim::address addr, uint32_t cpu, bool useless, long set, long way, uint8_t prefetch, champsim::address evicted_addr, uint32_t metadata_in)
{
...
void spp_ppf_4kb::PPF_Module::final_stats()
{
	if(SPP_DEBUG_PRINT) {
...
void spp_ppf_4kb::PPF_Module::SIGNATURE_TABLE::read_and_update_sig(page_type page, offset_type page_offset, uint32_t &last_sig, uint32_t &curr_sig, typename offset_type::difference_type &delta)
{
...
void spp_ppf_4kb::PPF_Module::PATTERN_TABLE::update_pattern(uint32_t last_sig, typename offset_type::difference_type curr_delta)
{
...
bool spp_ppf_4kb::PPF_Module::PREFETCH_FILTER::check(champsim::address check_addr, champsim::address base_addr, champsim::address ip, FILTER_REQUEST filter_request, typename offset_type::difference_type cur_delta, uint32_t last_sig, uint32_t curr_sig, uint32_t conf, int32_t sum, uint32_t depth)
{
...
void spp_ppf_4kb::PPF_Module::GLOBAL_REGISTER::update_entry(uint32_t pf_sig, uint32_t pf_confidence, offset_type pf_offset, typename offset_type::difference_type pf_delta) 
{
...
uint32_t spp_ppf_4kb::PPF_Module::GLOBAL_REGISTER::check_entry(offset_type page_offset)
{
    uint32_t max_conf = 0,
...
void spp_ppf_4kb::PPF_Module::get_perc_index(champsim::address base_addr, champsim::address ip, champsim::address ip_1, champsim::address ip_2, champsim::address ip_3, typename offset_type::difference_type cur_delta, uint32_t last_sig, uint32_t curr_sig, uint32_t confidence, uint32_t depth, uint64_t* perc_set)
{
...
int32_t	spp_ppf_4kb::PPF_Module::PERCEPTRON::perc_predict(champsim::address base_addr, champsim::address ip, champsim::address ip_1, champsim::address ip_2, champsim::address ip_3, typename offset_type::difference_type cur_delta, uint32_t last_sig, uint32_t curr_sig, uint32_t confidence, uint32_t depth)
{
	if(SPP_DEBUG_PRINT) {
		int sig_delta = (cur_delta < 0) ? (((-1) * cur_delta) + (1 << (SIG_DELTA_BIT - 1))) : cur_delta;
