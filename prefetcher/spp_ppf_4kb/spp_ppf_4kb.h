#ifndef SPP_PPF_4KB_H
#define SPP_PPF_4KB_H

#include <iostream>
#include <fstream>
using namespace std;

struct spp_ppf_4kb : public champsim::modules::prefetcher {
    ...
    block_in_page_extent() : dynamic_extent(champsim::data::bits{12}, champsim::data::bits{LOG2_BLOCK_SIZE}) {}
  	};
	struct page_extent : champsim::dynamic_extent {
    page_extent() : dynamic_extent(champsim::data::bits{64}, champsim::data::bits{12}) {}
  	};
  	using offset_type = champsim::address_slice<block_in_page_extent>;
	using page_type = champsim::address_slice<page_extent>;
	...
	public:
		...
		void read_and_update_sig(page_type page, offset_type page_offset, uint32_t &last_sig, uint32_t &curr_sig, typename offset_type::difference_type &delta);
	};
	...
	public:
		...
		page_type page_tracker[PAGES_TRACKED];
