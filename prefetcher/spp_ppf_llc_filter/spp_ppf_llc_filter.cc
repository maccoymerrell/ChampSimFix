...
#include <cmath>
#include "cache.h"
#include "spp_ppf_llc_filter.h"





void spp_ppf_llc_filter::prefetcher_initialize() 
{
	for(int a = 0; a < 30; a++)
		depth_track[a] = 0;
    ...
    fmt::print("---------SPP_PPF_WITH_LLC_FILTER------\n");

}

uint32_t spp_ppf_llc_filter::prefetcher_cache_operate(champsim::address addr, champsim::address ip, uint32_t cpu, uint8_t cache_hit, bool useful_prefetch, access_type type,
...
uint32_t spp_ppf_llc_filter::prefetcher_cache_fill(champsim::address addr, uint32_t cpu, bool useless, long set, long way, uint8_t prefetch, champsim::address evicted_addr, uint32_t metadata_in)
{
...
void spp_ppf_llc_filter::prefetcher_final_stats()
{
	if(SPP_DEBUG_PRINT) {
...
uint64_t spp_ppf_llc_filter::get_hash(uint64_t key)
{
...
void spp_ppf_llc_filter::SIGNATURE_TABLE::read_and_update_sig(champsim::page_number page, offset_type page_offset, uint32_t &last_sig, uint32_t &curr_sig, typename offset_type::difference_type &delta)
{
    uint32_t set = spp_ppf_llc_filter::get_hash(page.to<uint64_t>()) % ST_SET, match = ST_WAY;
...
void spp_ppf_llc_filter::PATTERN_TABLE::update_pattern(uint32_t last_sig, typename offset_type::difference_type curr_delta)
{
    // Update (sig, delta) correlation
    uint32_t set = spp_ppf_llc_filter::get_hash(last_sig) % PT_SET,
             match = 0;
...
bool spp_ppf_llc_filter::PREFETCH_FILTER::check(champsim::address check_addr, champsim::address base_addr, champsim::address ip, FILTER_REQUEST filter_request, typename offset_type::difference_type cur_delta, uint32_t last_sig, uint32_t curr_sig, uint32_t conf, int32_t sum, uint32_t depth)
{
    champsim::block_number cache_line{check_addr};
    uint64_t hash = spp_ppf_llc_filter::get_hash(cache_line.to<uint64_t>());
...
void spp_ppf_llc_filter::GLOBAL_REGISTER::update_entry(uint32_t pf_sig, uint32_t pf_confidence, offset_type pf_offset, typename offset_type::difference_type pf_delta) 
{
...
uint32_t spp_ppf_llc_filter::GLOBAL_REGISTER::check_entry(offset_type page_offset)
{
    uint32_t max_conf = 0,
...
void spp_ppf_llc_filter::get_perc_index(champsim::address base_addr, champsim::address ip, champsim::address ip_1, champsim::address ip_2, champsim::address ip_3, typename offset_type::difference_type cur_delta, uint32_t last_sig, uint32_t curr_sig, uint32_t confidence, uint32_t depth, uint64_t* perc_set)
{
...
int32_t	spp_ppf_llc_filter::PERCEPTRON::perc_predict(champsim::address base_addr, champsim::address ip, champsim::address ip_1, champsim::address ip_2, champsim::address ip_3, typename offset_type::difference_type cur_delta, uint32_t last_sig, uint32_t curr_sig, uint32_t confidence, uint32_t depth)
{
	if(SPP_DEBUG_PRINT) {
		int sig_delta = (cur_delta < 0) ? (((-1) * cur_delta) + (1 << (SIG_DELTA_BIT - 1))) : cur_delta;
