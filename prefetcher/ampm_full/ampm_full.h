#ifndef PREFETCHER_AMPM_FULL_H
#define PREFETCHER_AMPM_FULL_H
...
class ampm_full : public champsim::modules::prefetcher
{
  public:
    struct AMPM_Full_Module {
      CACHE* intern_;
      ...
      static constexpr unsigned int AMPM_FULL_PAGE_BITS = 12;
      ...
      static constexpr bool DO_NEGATIVE = true;
      ...
      std::array<int64_t,16> PREFETCH_DEGREES_BW = {0,0,1,1,1,1,1,1,2,2,2,4,4,4,8,8};
      std::array<int64_t,16> PREFETCH_DEGREES_USEFULNESS = {1,1,2,2,2,3,3,3,4,4,8,8,12,12,16,16};
      std::array<int64_t,16> PREFETCH_DROP_CHANCE_USEFULNESS = {125,120,120,80,65,50,35,20,5,0,0,0,0,0,0,0}; //out of 128
      ...
      struct page_extent : champsim::dynamic_extent {
        page_extent() : dynamic_extent(champsim::data::bits{64}, champsim::data::bits{AMPM_FULL_PAGE_BITS}) {}
      };
      using page = champsim::address_slice<page_extent>;

      struct block_in_page_extent : champsim::dynamic_extent {
        block_in_page_extent() : dynamic_extent(champsim::data::bits{AMPM_FULL_PAGE_BITS}, champsim::data::bits{LOG2_BLOCK_SIZE}) {}
      };
      ...
      struct region_type {
        ...
        explicit region_type(page allocate_vpn, uint64_t last_access_time_)
          : vpn(allocate_vpn), next_vpn(page{}), prev_vpn(page{}), access_map((1 << AMPM_FULL_PAGE_BITS) / BLOCK_SIZE), prefetch_map((1 << AMPM_FULL_PAGE_BITS) / BLOCK_SIZE), last_access_time(last_access_time_), pf_trigger(((1 << AMPM_FULL_PAGE_BITS) / BLOCK_SIZE))
        {
        }
      };


      struct ampm_full_indexer {
        auto operator()(const region_type& entry) const { return entry.vpn; }
      };
      champsim::msl::lru_table<region_type,ampm_full_indexer,ampm_full_indexer> regions{REGION_SETS,REGION_WAYS};

      champsim::msl::lru_table<region_type,ampm_full_indexer,ampm_full_indexer> llc_regions{LLC_REGION_SETS,LLC_REGION_WAYS};
      ...
      struct pattern_type {
        ...
        sppam::msl::conf_table<uint64_t,pattern_conf_indexer,pattern_conf_indexer> prediction_table{PATTERN_CONF_SETS,PATTERN_CONF_SETS_WAYS};
