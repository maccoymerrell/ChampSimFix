...
template <typename T>
auto ampm_full::AMPM_Full_Module::page_and_offset(T addr) -> std::pair<page, block_in_page>
{
  return std::pair{page{addr}, block_in_page{addr}};
}

std::pair<uint64_t,uint64_t> ampm_full::AMPM_Full_Module::get_patterns(page pn, block_in_page page_offset) {
  ...
  if(region.has_value()) {
    ...
    for(int i = page_offset.to<int64_t>(); i < page_offset.to<int64_t>() + PATTERN_SIZE; i++) {
      pos_access_pattern = (pos_access_pattern << 1) | (uint64_t(shadow_access_map.at(i)));
    }
    for(int i = page_offset.to<int64_t>() + (PATTERN_SIZE*2); i > page_offset.to<int64_t>() + PATTERN_SIZE; i--) {
...
bool ampm_full::AMPM_Full_Module::check_pagemap(champsim::address addr, bool prefetch)
{
...
std::pair<uint64_t,bool> ampm_full::AMPM_Full_Module::get_prefetch_pattern(uint64_t pattern, int order, bool negative) {
...
void ampm_full::AMPM_Full_Module::increment_access_pattern(uint64_t pattern, uint64_t prediction, int order, bool negative) {
  auto temp_pattern = pattern_type{pattern & ((1 << (PATTERN_SIZE >> order)) - 1)};
  auto demand_pattern = negative && SEPARATE_NEGATIVE_TABLES ?  negative_pattern_tables.at(order).check_hit(temp_pattern) : pattern_tables.at(order).check_hit(temp_pattern);
...
void ampm_full::AMPM_Full_Module::initialize(CACHE* cache) {
  intern_ = cache;
...
void ampm_full::AMPM_Full_Module::add_to_pagemap(champsim::address addr, bool prefetch, champsim::address ip, uint64_t pf_trigger, page prev_page, bool direction) {
...
void ampm_full::AMPM_Full_Module::add_to_llc_pagemap(champsim::address addr) {
  auto [current_pn, page_offset] = page_and_offset(addr);
...
bool ampm_full::AMPM_Full_Module::check_llc_pagemap(champsim::address addr) {
  auto [current_pn, page_offset] = page_and_offset(addr);
  auto temp_region = region_type{current_pn,0};
...
void ampm_full::AMPM_Full_Module::remove_from_pagemap(champsim::address addr, bool prefetch) {
  auto [current_pn, page_offset] = page_and_offset(addr);
...
std::pair<ampm_full::AMPM_Full_Module::page,bool> ampm_full::AMPM_Full_Module::get_prev_page(uint32_t stream_id, page current_vpn) {
  //check cpt table
  if(stream_id == 0 || stream_id == 256)
    return {page{},true};

  auto temp_cpt = cross_page_tracker_type{stream_id & 0xff,current_vpn};
  ...
  if(cpt_entry.has_value()) {
    ...
    temp_cpt.forward = stream_id > 256;
    cpt_table.fill(temp_cpt);
    return {page{}, true};
  }
}

uint32_t ampm_full::prefetcher_cache_operate(champsim::address addr, champsim::address ip, bool cache_hit, bool useful_prefetch, access_type type, uint32_t metadata_in)
{
  ...
  auto [prev_page, dir] = engine.get_prev_page(metadata_in,AMPM_Full_Module::page{addr});
  ...
  engine.do_prefetch(addr,ip,cache_hit,useful_prefetch,type,0);

  return metadata_in;
}

uint32_t ampm_full::prefetcher_cache_fill(champsim::address addr, long set, long way, bool prefetch, champsim::address evicted_addr, uint32_t metadata_in)
{
...
void ampm_full::AMPM_Full_Module::scrape_region(champsim::address addr) {
...
int ampm_full::AMPM_Full_Module::get_momentum(champsim::address addr) {
  auto [pn, page_offset] = page_and_offset(addr);
  auto region = regions.check_hit(region_type{pn,intern_->current_cycle()});
  if(region.has_value()) {
...
void ampm_full::AMPM_Full_Module::do_prefetch(champsim::address addr, champsim::address ip, uint8_t cache_hit, bool useful_prefetch, access_type type, uint32_t metadata_in) {
        ...
        for(int i = PATTERN_SIZE; i >= int(MIN_PATTERN_SIZE); i/=2) {
          ...
          while(true) {
            ...
            if((intern_->current_cycle() % 128) < PREFETCH_DROP_CHANCE_USEFULNESS.at(current_usefulness)) {
              if(!use_pattern_confidence() && PROB_DROP_PREFETCHES) {
                prefetches_dropped++;
                if((!in_warmup) && DO_DEBUG)
                fmt::print("\t\tDropped!\n");
                break;
            ...
            for(int j = 0; j < i; j++) {
              ...
              if(pos_prefetch_pattern & (1 << (i - 1 - j))) {
                ...
                if(!check_pagemap(champsim::address{pos_step_addr},true)) {
                  if((pf_issued < free_space) || !check_llc_pagemap(pos_step_addr)) { //filter LLC prefetches
            ...
            for(int j = 0; j < i; j++) {
              ...
              if(neg_prefetch_pattern & (1 << (i - 1 - j))) {
                ...
                if(!check_pagemap(champsim::address{neg_step_addr},true)) {
                  if((pf_issued < free_space) || !check_llc_pagemap(neg_step_addr) ) {
...
void ampm_full::AMPM_Full_Module::print_patterns() {
...
uint64_t ampm_full::AMPM_Full_Module::get_state_bits() {
  //Assuming 48-bit max address
  //calculate total bits needed for AMPM_Full_Module
  uint64_t total_bits = 0;

  //start with region table
  uint64_t region_table_bits = (48 - champsim::lg2(AMPM_FULL_PAGE_BITS)) * 3; //page nums
  region_table_bits += ((1 << AMPM_FULL_PAGE_BITS) / BLOCK_SIZE)*2; //bitmaps
  region_table_bits += SCRAPE_ON_IDLE ? champsim::lg2(SCRAPE_IDLE_TIME) : 0; //scrape count
  region_table_bits += REGION_PATTERN_TAG ? (((1 << AMPM_FULL_PAGE_BITS) / BLOCK_SIZE) * PATTERN_SIZE) : 0;
  //region_table_bits += 4 //momentum (DISABLED)
  //region_table_bits += champsim::lg2(((1 << AMPM_FULL_PAGE_BITS) / BLOCK_SIZE)); //last block
  ...
  //CPT table
  uint64_t cpt_bits = 8 + 1 + (48 - champsim::lg2(AMPM_FULL_PAGE_BITS));
...
bool ampm_full::AMPM_Full_Module::use_pattern_confidence() {
  static bool using_pattern_confidence = false;
  if(!ADAPTIVE_USEFULNESS)
    return GLOBAL_OR_PATTERN_USEFULNESS;

  if(region_lifespan_index >= PATTERN_USEFULNESS_CUTOFF) {
    if(!using_pattern_confidence)
      fmt::print("Switched to pattern confidence with Region Lifespan: {}\n",region_lifespan_index);
    using_pattern_confidence = true;
    return true;
  }
  else {
    if(using_pattern_confidence)
      fmt::print("Switched to global confidence with Region Lifespan: {}\n",region_lifespan_index);
    using_pattern_confidence = false;
    return false;
  }
}

void ampm_full::AMPM_Full_Module::modify_pattern_usefulness(champsim::address addr, bool useful) {
  auto [pn, page_offset] = page_and_offset(addr);
  auto region = regions.check_hit(region_type{pn,intern_->current_cycle()});
  if(!region.has_value()) {
    if(!useful)
...
void ampm_full::prefetcher_cycle_operate() {
...
int ampm_full::AMPM_Full_Module::set_prefetch_degree(uint64_t pattern, int order, bool negative, int prev_usefulness = 15) {
  current_bw_utilization = get_dram_bw();
  if((GLOBAL_OR_PATTERN_USEFULNESS && !ADAPTIVE_USEFULNESS) || (use_pattern_confidence() && ADAPTIVE_USEFULNESS)) {
    ...
    if(demand_pattern.has_value()) {
      int new_usefulness = do_4_bit_mult(demand_pattern->usefulness,prev_usefulness);
      current_pf_degree = std::max(PREFETCH_DEGREES_USEFULNESS.at(new_usefulness) - PREFETCH_DEGREES_BW.at(current_bw_utilization), 0l);
      return std::min(new_usefulness + 1,15);
    } else {
      int new_usefulness = do_4_bit_mult(prev_usefulness,global_usefulness_index);
      current_pf_degree = std::max(PREFETCH_DEGREES_USEFULNESS.at(new_usefulness) - PREFETCH_DEGREES_BW.at(current_bw_utilization), 0l );
      return std::min(new_usefulness+1,15);
    }
  }
...
void ampm_full::AMPM_Full_Module::decrease_usefulness_counter() {
  /*
...
void ampm_full::AMPM_Full_Module::increase_usefulness_counter() {
  /*
...
int ampm_full::AMPM_Full_Module::do_4_bit_mult(int op1, int op2) {
...
void ampm_full::prefetcher_final_stats() {
