...
uint32_t orap_ampm::prefetcher_cache_operate(champsim::address addr, champsim::address ip, uint32_t cpu, uint8_t cache_hit, bool useful_prefetch, access_type type,
                                             uint32_t metadata_in, uint32_t metadata_hit)
{
  //if(!intern_->warmup && metadata_in == ASD_ID && !cache_hit) {
  //  fmt::print("AMPM Prefetch reported\n");
  //}
  ...
  if(metadata_in != ASD_ID)
    ASD_Modules.at(cpu).add_to_pagemap(addr,false);
  ...
  if(metadata_in != ASD_ID && !DISABLE_ASD) {
    uint8_t confidence = get_confidence_from_ip_hash(get_hash(ip.to<uint64_t>()),cpu,true);
    double asd_thresh = get_asd_thresh(confidence,cpu);
    ASD_Modules.at(cpu).do_prefetch(intern_,addr,ip,cpu,cache_hit,useful_prefetch,type,ASD_ID,asd_thresh,true,true);
    if(!intern_->warmup) {
      next_line_issued += asd_thresh;
      tally_prefetch(confidence,cpu,asd_thresh,true,false);
    }
  }
...
bool orap_ampm::update_row_open_table(champsim::address addr) {
...
uint8_t orap_ampm::modify_confidence(uint8_t conf, uint8_t amnt, bool increment) {
  if(increment) {
...
uint8_t orap_ampm::modify_confidence(uint8_t conf, double factor) {
...
void orap_ampm::increase_confidence_useful(champsim::address ip, champsim::address addr, uint32_t cpu, bool coprefetch) {
...
void orap_ampm::increase_confidence_opened(champsim::address ip, champsim::address addr, uint32_t cpu, bool coprefetch) {
  //fmt::print("Increasing confidence, row: {} rb: {} col: {}\n",row,rb,col);
...
std::size_t orap_ampm::get_depth(uint8_t conf, uint32_t cpu) {
  uint8_t threshold = 0;
...
void orap_ampm::tally_prefetch(uint8_t conf, uint32_t cpu, uint8_t amount, bool coprefetch, bool row) {
  uint8_t threshold = 0;
  for (auto thres : THRESH) {
    threshold++;
    if(conf < thres)
  ...
  if(coprefetch) {
    cp_ip_at_conf.at(cpu).at(threshold-1) += amount;
  } else {
    if (row)
      orap_row_at_conf.at(cpu).at(threshold-1) += amount;
    else
      orap_ip_at_conf.at(cpu).at(threshold-1) += amount;
  }
}

uint8_t orap_ampm::get_squash_chance(uint8_t conf, uint32_t cpu) {
  uint8_t threshold = 0;
  uint8_t new_conf = conf;
  for (auto thres : THRESH) {
    threshold++;
    if(new_conf < thres)
      break;
  ...
  //if(new_conf != conf)
  //  fmt::print("Old Confidence: {} Modifier: {} New Confidence: {} Depth: {}\n",conf,variable_buffer_conf[cpu],new_conf,DEPTHS.at(threshold-1));
  int adjusted_min = 127 - (global_usefulness_buffer[cpu] * global_usefulness_buffer[cpu] * global_usefulness_buffer[cpu] * 127);
  adjusted_min = std::max(adjusted_min,1);
  adjusted_min = std::min(127,adjusted_min);
  return std::max(adjusted_min,127 - CHANCE.at(threshold - 1));
}

double orap_ampm::get_asd_thresh(uint8_t conf, uint32_t cpu) {
  uint8_t threshold = 0;
  for (auto thres : THRESH) {
    threshold++;
    if(conf < thres)
...
void orap_ampm::update_walker(champsim::address addr, uint32_t cpu, champsim::address ip, bool is_blacklisted) {
  std::size_t row = MEMORY_CONTROLLER::DRAM_CONTROLLER.value()->dram_get_row(addr);
  ...
  bool used_row_conf = ((row_confidence > ip_confidence) && USE_ROW_CONF) || !USE_IP_CONF;
  uint8_t confidence = USE_ROW_CONF && USE_IP_CONF ? (std::max(ip_confidence,row_confidence)) :
                       USE_ROW_CONF                ? row_confidence :
                       USE_IP_CONF                 ? ip_confidence :
                       0;
  ...
  if((intern_->current_cycle() % 128) < squash_chance) {
    if(!intern_->warmup)
      streams_squashed++;
    return;
  }
  ...
  if(USE_PREFETCH_QUEUE) {
    if(amount_to_prefetch != 0) {
      ...
      if(next_col >= 0 && next_col < (1 << (column_bits.size())))
        add_to_pq(prefetch_queue_entry(addr,
                                      amount_to_prefetch,
                                      col_stride,
                                      ip,
                                      cpu,
    ...
    for(int i = col + col_stride; (col_stride > 0 ? (i < (1 << (column_bits.size())) && i <= (int)col + amount_to_prefetch) : (i >= 0 && i >= (int)col - amount_to_prefetch)); i += col_stride) {
      ...
      if(!pm) {
          ...
          if(success) {
            ...
            ASD_Modules.at(cpu).add_to_pagemap(compose_base_and_column(addr,i),true);
...
champsim::address orap_ampm::compose_base_and_column(champsim::address base, uint64_t column) {
...
void orap_ampm::prefetcher_initialize() {
  if(NUM_CPUS == 1) {
  ...
  for(int i = 0; i < NUM_CPUS; i++) {
    ...
    cp_ip_at_conf.push_back(std::vector<uint32_t>(THRESH.size(),0));
    orap_ip_at_conf.push_back(std::vector<uint32_t>(THRESH.size(),0));
    orap_row_at_conf.push_back(std::vector<uint32_t>(THRESH.size(),0));
  }
  ...
  //for(int i = 0; i < NUM_CPUS; i++) {
  //  ASD_Modules.emplace_back(ampm());
  //}
  ASD_Modules.resize(NUM_CPUS);
...
uint32_t orap_ampm::prefetcher_cache_fill(champsim::address addr, champsim::address ip, uint32_t cpu, bool useless, long set, long way, uint8_t prefetch, champsim::address evicted_addr, uint32_t metadata_in, uint32_t metadata_evict, uint32_t cpu_evict)
{
  ...
  //sample stuff
  if(prefetch && (metadata_in == ASD_ID || metadata_in == BUFFER_ID)) {
...
void orap_ampm::prefetcher_cycle_operate() {
  epoch_cycle_counter++;
...
void orap_ampm::prefetcher_final_stats() {
  ...
  fmt::print("ISSUED PER CORE: (CORE : CONF, ORAP ROW ISSUED, ORAP IP ISSUED, CP IP ISSUED)\n");
  for(int c = 0; c < NUM_CPUS; c++) {
    for (int i = 0; i < DEPTHS.size(); i++) {
      fmt::print("\t{}: {} {} {} {}\n",c,THRESH[i],orap_row_at_conf.at(c).at(i), orap_ip_at_conf.at(c).at(i), cp_ip_at_conf.at(c).at(i));
    }
  }
...
void orap_ampm::reset_ip_blacklist() {
  for(int i = 0; i < NUM_CPUS; i++) {
    ip_blacklist_table[i].flush();
  }
}

bool orap_ampm::is_ip_blacklisted(champsim::address ip, uint32_t cpu) {
...
void orap_ampm::update_ip_blacklist(std::size_t collision_type, champsim::address ip, uint32_t cpu, uint32_t victim_cpu, uint32_t target_prefetcher, uint32_t victim_prefetcher) {
  ip_blacklist_counter bc{ip};
...
std::vector<uint16_t> orap_ampm::get_ip_hash_from_row(uint32_t row, uint32_t cpu) {
  row_walker rw{row};
...
uint8_t orap_ampm::get_confidence_from_ip_hash(uint16_t ip_hash, uint32_t cpu, bool coprefetch) {
  ip_tracker it{ip_hash};
...
uint8_t orap_ampm::get_confidence_from_row(champsim::address addr, uint32_t cpu) {
  std::size_t row = MEMORY_CONTROLLER::DRAM_CONTROLLER.value()->dram_get_row(addr);
  row_walker rw{row};
...
void orap_ampm::log_ip_hash_to_row(uint32_t row, uint16_t ip_hash, uint32_t cpu) {
  row_walker rw{row};
...
void orap_ampm::add_to_pq(prefetch_queue_entry pqe) {
  
    for(int i = 1; i <= pqe.length; i++) {
      ...
      if(pf_queue[cpu_to_use][rb].size() >= PREFETCH_SUBQUEUE_LIMIT) {
        ...
        ASD_Modules.at(pqe.cpu).remove_from_pagemap(pf_queue[cpu_to_use][rb][entry_pos].start_addr,true);
      ...
      ASD_Modules.at(pqe.cpu).add_to_pagemap(temp_pqe.start_addr,true);
    }
}

void orap_ampm::clear_from_pq(champsim::address addr, uint32_t cpu) {
