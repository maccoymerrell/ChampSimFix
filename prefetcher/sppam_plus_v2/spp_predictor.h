...
public:
  ...
  void predict_path(int64_t block, uint32_t start_sig, uint32_t s0)
  {
    ...
    for (uint32_t d = 0; d < eff_la; ++d) {
      ...
      // Perceptron final gate (engine 2 = SPP/delta signature). SPP is PC-agnostic (pc=0); sig = the delta signature.
      if (sink_->perc_gate(static_cast<uint64_t>(pred), 2, static_cast<int>(d), 0, static_cast<int>(conf * 15.0), s)) {
        if (sink_->issue_prefetch(static_cast<uint64_t>(pred), true, /*from_spp=*/true, /*benefit=*/conf, /*gen_tag=*/1u | ((d > 15u ? 15u : d) << 1)))
          sink_->perc_note_issue_ext(static_cast<uint64_t>(pred));
      }
      s = advance(s, best.delta); // the signature path advances whether or not this block was issued
    }
  }
