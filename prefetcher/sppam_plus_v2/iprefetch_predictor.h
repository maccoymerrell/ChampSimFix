...
public:
  ...
  void operate(uint64_t ip_block, uint64_t phys_block, const IssueFn& issue)
  {
    ...
    // v2 sequential fallback: the confidence-gated edge walk follows control flow but drops the short-range
    // SEQUENTIAL residual of multi-line basic blocks. Issue this code page's next instr_nextn physical blocks
    // (the shared residency filter squashes the overlap with the edge next-line; DSE: next-2 ~= full SPPAM).
