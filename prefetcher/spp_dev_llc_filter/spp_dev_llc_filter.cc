#include "spp_dev_llc_filter.h"

#include <cassert>
#include <iostream>

void spp_dev_llc_filter::prefetcher_initialize()
{
...
void spp_dev_llc_filter::prefetcher_cycle_operate() {}


uint32_t spp_dev_llc_filter::prefetcher_cache_operate(champsim::address addr, champsim::address ip, uint32_t cpu, uint8_t cache_hit, bool useful_prefetch, access_type type, uint32_t metadata_in)
{
  ...
  typename spp_dev_llc_filter::offset_type::difference_type delta = 0;
  std::vector<typename spp_dev_llc_filter::offset_type::difference_type> delta_q(intern_->get_mshr_size());
  ...
  FILTER.check(addr, spp_dev_llc_filter::L2C_DEMAND);
      ...
      if (confidence_q[i] >= PF_THRESHOLD) {
        ...
        if (champsim::page_number{pf_addr} == page) { // Prefetch request is in the same physical page
          if (FILTER.check(pf_addr, ((confidence_q[i] >= FILL_THRESHOLD) ? spp_dev_llc_filter::SPP_L2C_PREFETCH : spp_dev_llc_filter::SPP_LLC_PREFETCH)) && ((confidence_q[i] >= FILL_THRESHOLD) || !FILTER.filter_raf.check(pf_addr,intern_->current_cycle(),false))) {
            ...
            //fmt::print("successfully prefetched address {}\n",pf_addr);
          ...
          if constexpr (GHR_ON) {
            ...
            GHR.update_entry(curr_sig, confidence_q[i], spp_dev_llc_filter::offset_type{pf_addr}, delta_q[i]);
          }
        }
...
uint32_t spp_dev_llc_filter::prefetcher_cache_fill(champsim::address addr, uint32_t cpu, bool useless, long set, long way, uint8_t prefetch, champsim::address evicted_addr, uint32_t metadata_in)
{
  if constexpr (FILTER_ON) {
    if constexpr (SPP_DEBUG_PRINT) {
      std::cout << std::endl;
    }
    FILTER.check(evicted_addr, spp_dev_llc_filter::L2C_EVICT);
  }

  return metadata_in;
}

void spp_dev_llc_filter::prefetcher_final_stats() {}


// TODO: Find a good 64-bit hash function
uint64_t spp_dev_llc_filter::get_hash(uint64_t key)
{
...
void spp_dev_llc_filter::SIGNATURE_TABLE::read_and_update_sig(champsim::address addr, uint32_t& last_sig, uint32_t& curr_sig, typename offset_type::difference_type& delta)
{
...
void spp_dev_llc_filter::PATTERN_TABLE::update_pattern(uint32_t last_sig, typename offset_type::difference_type curr_delta)
{
...
void spp_dev_llc_filter::PATTERN_TABLE::read_pattern(uint32_t curr_sig, std::vector<typename offset_type::difference_type>& delta_q, std::vector<uint32_t>& confidence_q,
...
bool spp_dev_llc_filter::PREFETCH_FILTER::check(champsim::address check_addr, FILTER_REQUEST filter_request)
{
  ...
  case spp_dev_llc_filter::SPP_L2C_PREFETCH:
  ...
  case spp_dev_llc_filter::SPP_LLC_PREFETCH:
    if ((valid[quotient] || useful[quotient]) && remainder_tag[quotient] == remainder) {
      if constexpr (SPP_DEBUG_PRINT) {
        std::cout << "[FILTER] " << __func__ << " line is already in the filter check_addr: " << check_addr << " cache_line: " << cache_line;
        std::cout << " quotient: " << quotient << " valid: " << valid[quotient] << " useful: " << useful[quotient] << std::endl;
  ...
  case spp_dev_llc_filter::L2C_DEMAND:
  ...
  case spp_dev_llc_filter::L2C_EVICT:
...
void spp_dev_llc_filter::GLOBAL_REGISTER::update_entry(uint32_t pf_sig, uint32_t pf_confidence, offset_type pf_offset, typename offset_type::difference_type pf_delta)
{
...
uint32_t spp_dev_llc_filter::GLOBAL_REGISTER::check_entry(offset_type page_offset)
{
