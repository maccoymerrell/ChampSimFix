...
std::vector<berti_fixed::HistoryTable*> berti_fixed::historyt;
std::vector<berti_fixed::LatencyTable*> berti_fixed::latencyt;
std::vector<berti_fixed::ShadowCache*> berti_fixed::scache;
uint64_t berti_fixed::others = 0;

uint8_t berti_fixed::LatencyTable::add(uint64_t addr, uint64_t tag, bool pf, uint64_t cycle)
{
  /*
...
uint64_t berti_fixed::LatencyTable::del(uint64_t addr)
{
  /*
...
uint64_t berti_fixed::LatencyTable::get(uint64_t addr)
{
  /*
...
uint64_t berti_fixed::LatencyTable::get_tag(uint64_t addr)
{
  /*
...
/******************************************************************************/
bool berti_fixed::ShadowCache::add(uint32_t set, uint32_t way, uint64_t addr, bool pf, uint64_t lat)
{
  /*
...
bool berti_fixed::ShadowCache::get(uint64_t addr)
{
  /*
   * Parameters:
   *      - addr: cache block v_addr
...
void berti_fixed::ShadowCache::set_pf(uint64_t addr, bool pf)
{
  /*
   * Parameters:
   *      - addr: cache block v_addr
...
bool berti_fixed::ShadowCache::is_pf(uint64_t addr)
{
  /*
   * Parameters:
   *      - addr: cache block v_addr
...
uint64_t berti_fixed::ShadowCache::get_latency(uint64_t addr)
{
  /*
   * Init shadow cache
   *
...
/******************************************************************************/
void berti_fixed::HistoryTable::add(uint64_t tag, uint64_t addr, uint64_t cycle)
{
  /*
...
uint16_t berti_fixed::HistoryTable::get_aux(uint32_t latency, 
...
uint16_t berti_fixed::HistoryTable::get(uint32_t latency, uint64_t tag, uint64_t act_addr,
...
/******************************************************************************/
void berti_fixed::increase_conf_tag(uint64_t tag)
{
  /*
...
void berti_fixed::add(uint64_t tag, int64_t delta)
{
  /*
   * Save the new information into the history table
   *
...
uint8_t berti_fixed::get(uint64_t tag, std::vector<delta_t> &res)
{
  /*
   * Save the new information into the history table
   *
...
void berti_fixed::find_and_update(uint64_t latency, uint64_t tag, uint64_t cycle, 
    uint64_t line_addr)
{ 
...
bool berti_fixed::compare_rpl(delta_t a, delta_t b)
{
...
bool berti_fixed::compare_greater_delta(delta_t a, delta_t b)
{
...
bool berti_fixed::get_region_history(champsim::address addr) {
  //disabled, this is an improvement of our version of berti, not used by standard berti
...
void berti_fixed::remove_region_history(champsim::address addr) {
  region_type::page pn = region_type::page(addr);
  region_type::block_in_page po = region_type::block_in_page{addr};
  auto temp_region = region_type{pn};
...
void berti_fixed::add_region_history(champsim::address addr) {
  region_type::page pn = region_type::page(addr);
  region_type::block_in_page po = region_type::block_in_page{addr};
  auto temp_region = region_type{pn};
...
uint64_t berti_fixed::ip_hash(uint64_t ip)
{
  /*
   * IP hash function
   */
...
/*                        Cache Functions                                     */
/******************************************************************************/
void berti_fixed::prefetcher_initialize() 
{
...
void berti_fixed::prefetcher_cycle_operate()
{}

uint32_t berti_fixed::prefetcher_cache_operate(champsim::address addr, champsim::address ip, bool cache_hit, bool useful_prefetch, access_type type, uint32_t metadata_in)
{
...
uint32_t berti_fixed::prefetcher_cache_fill(champsim::address addr, long set, long way, bool prefetch, champsim::address evicted_addr, uint32_t metadata_in)
{
...
void berti_fixed::prefetcher_final_stats()
{
