...
void berti_plus::prefetcher_initialize() 
{
  ...
  uint64_t additional_state = (((BERTI_REGION_HISTORY_SETS * BERTI_REGION_HISTORY_WAYS)*((48-region_type::PAGE_BITS) + (1 << (region_type::PAGE_BITS - LOG2_BLOCK_SIZE))))/8) + 1;
  uint64_t vanilla_state = 2612;
  auto total_state = champsim::data::bytes{vanilla_state + additional_state};
  fmt::print("[{}] Berti+ Initialized. Total State: {}\n",intern_->NAME,champsim::data::kibibytes{total_state});
...
void berti_plus::prefetcher_final_stats()
{
  ...
  std::ofstream file("berti_region_coverage.txt", std::ios::out | std::ios::trunc);
  for(auto& entry : region_coverage_table.get_blocks()) {
    file << "Page: " << entry.data.vpn << std::endl;
    //as a bitvector
    for(size_t i = 0; i < entry.data.access_map.size(); i++) {
      file << (entry.data.access_map[i] ? "1" : "0");
    }
    file << std::endl;
    for(size_t i = 0; i < entry.data.prefetch_map.size(); i++) {
      file << (entry.data.prefetch_map[i] ? "1" : "0");
    }
    file << std::endl;
    for(size_t i = 0; i < entry.data.delta_map.size(); i++) {
      file << entry.data.delta_map[i] << " ";
    }
    file << std::endl;
  }
  file.close();
