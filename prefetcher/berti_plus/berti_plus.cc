#include "berti_plus.h"
#include <algorithm>

// Register this prefetcher under the name "berti_plus" for use in config files.
champsim::modules::prefetcher::register_module<berti_plus> berti_plus_register("berti_plus");

// Berti+ takes no tunable config parameters; all knobs live in
// berti_plus_parameters.h. The base prefetcher is default-constructed and its
// parent cache pointer is bound by the framework after construction.
berti_plus::berti_plus(champsim::modules::ModuleBuilder builder) : cache_(builder.get_parent<champsim::modules::cache_module>()) {}
...
uint64_t berti_plus::LatencyTable::get(uint64_t addr)
{
  /*
...
uint64_t berti_plus::LatencyTable::get_tag(uint64_t addr)
{
  /*
...
bool berti_plus::ShadowCache::is_pf(uint64_t addr)
{
  /*
   * Parameters:
   *      - addr: cache block v_addr
...
uint64_t berti_plus::ShadowCache::get_latency(uint64_t addr)
{
  /*
   * Init shadow cache
   *
...
/******************************************************************************/
void berti_plus::HistoryTable::add(uint64_t tag, uint64_t addr, uint64_t cycle)
{
  /*
...
uint16_t berti_plus::HistoryTable::get_aux(uint32_t latency, 
...
uint16_t berti_plus::HistoryTable::get(uint32_t latency, uint64_t tag, uint64_t act_addr,
...
/******************************************************************************/
void berti_plus::increase_conf_tag(uint64_t tag)
{
  /*
...
void berti_plus::add(uint64_t tag, int64_t delta)
{
  /*
   * Save the new information into the history table
   *
...
uint8_t berti_plus::get(uint64_t tag, std::vector<delta_t> &res)
{
  /*
   * Save the new information into the history table
   *
...
void berti_plus::find_and_update(uint64_t latency, uint64_t tag, uint64_t cycle, 
    uint64_t line_addr)
{ 
...
bool berti_plus::compare_rpl(delta_t a, delta_t b)
{
...
bool berti_plus::compare_greater_delta(delta_t a, delta_t b)
{
...
bool berti_plus::get_region_history(champsim::address addr) {
  region_type::page pn = region_type::page(addr);
  region_type::block_in_page po = region_type::block_in_page{addr};
  auto temp_region = region_type{pn};
...
void berti_plus::remove_region_history(champsim::address addr) {
  region_type::page pn = region_type::page(addr);
  region_type::block_in_page po = region_type::block_in_page{addr};
  auto temp_region = region_type{pn};
...
void berti_plus::add_region_history(champsim::address addr) {
  region_type::page pn = region_type::page(addr);
  region_type::block_in_page po = region_type::block_in_page{addr};
  auto temp_region = region_type{pn};
...
uint64_t berti_plus::ip_hash(uint64_t ip)
{
  /*
   * IP hash function
   */
...
/*                        Cache Functions                                     */
/******************************************************************************/
void berti_plus::prefetcher_initialize() 
{
  
  // Calculate latency table size
  uint64_t latency_table_size = cache_->get_mshr_size();
  for (auto const &i : cache_->get_rq_size()) latency_table_size += i;
  for (auto const &i : cache_->get_wq_size()) latency_table_size += i;
  for (auto const &i : cache_->get_pq_size()) latency_table_size += i;


  //fix this
  latencyt = new LatencyTable(latency_table_size);
  scache = new ShadowCache(static_cast<int>(cache_->num_sets()), static_cast<int>(cache_->num_ways()));
  historyt = new HistoryTable();

  static const bool berti_norf = (std::getenv("BERTI_NO_REGION_FILTER") != nullptr);
  uint64_t additional_state = berti_norf ? 0 : ((((BERTI_REGION_HISTORY_SETS * BERTI_REGION_HISTORY_WAYS)*((48-region_type::PAGE_BITS) + (1 << (region_type::PAGE_BITS - LOG2_BLOCK_SIZE))))/8) + 1);
  uint64_t vanilla_state = 2612;
  auto total_state = champsim::data::bytes{vanilla_state + additional_state};
  fmt::print("[{}] Berti+ Initialized. Total State: {}\n",cache_->NAME,champsim::data::kibibytes{total_state});
  //std::cout << "Berti Prefetcher" << std::endl;
  //cache_->internal_PQ.set_timeout(1500);
/*
# ifdef NO_CROSS_PAGE
...
void berti_plus::prefetcher_cycle_operate()
{

}

uint32_t berti_plus::prefetcher_cache_operate(champsim::address addr, champsim::address ip, bool cache_hit, bool useful_prefetch, access_type type, uint32_t metadata_in)
{
  // We select the structures for every cpu
  LatencyTable* tlatencyt = latencyt;
  ...
  if (!cache_hit) // This is a miss
  {
    ...
    tlatencyt->add(line_addr.to<uint64_t>(), ip_hash, false, cache_->current_cycle()); // Add @ to latency
    thistoryt->add(ip_hash, line_addr.to<uint64_t>(), cache_->current_cycle()); // Add to the table
  } else if (cache_hit && tscache->is_pf(line_addr.to<uint64_t>())) // Hit bc prefetch
  {
    ...
    find_and_update(latency, ip_hash, cache_->current_cycle() & TIME_MASK, line_addr.to<uint64_t>());
    thistoryt->add(ip_hash, line_addr.to<uint64_t>(), cache_->current_cycle() & TIME_MASK);
  } else
  {
    if constexpr (champsim::debug_print) 
  ...
  //added to tell the L2C that these prefetches are the same trigger (and presumably crossed a page)
  if(prev_stream_page != champsim::page_number{line_addr}) {
    stream_id++;
    prev_stream_page = champsim::page_number{line_addr};
  }
  if(stream_id >= 256)
    stream_id = 1;
  bool is_first = true;
  for (auto i: deltas)
  {
    ...
    static const bool berti_norf = (std::getenv("BERTI_NO_REGION_FILTER") != nullptr);
    if(!berti_norf && get_region_history(p_addr))
      continue;
...
# endif
    ...
    float mshr_load = cache_->get_mshr_occupancy_ratio() * 100;
    ...
    int metadata = std::abs(i.delta) < 64 ? ((stream_id & BERTI_STREAM_ID_MASK) | ((uint32_t(i.delta > 0)) << BERTI_DIRECTION_BIT)) : 0;
    if(is_first)
      metadata |= (ip.to<uint64_t>() & BERTI_IP_MASK) << BERTI_IP_ENCODING_OFFSET;
    //is_first = false;
    ...
    if (prefetch_line(p_addr, fill_this_level, metadata))
    {
      ...
      if (fill_this_level)
      {
        ...
        {
          tlatencyt->add(p_b_addr.to<uint64_t>(), ip_hash, true, cache_->current_cycle());
        }
      }
    }
  }
...
uint32_t berti_plus::prefetcher_cache_fill(champsim::address addr, long set, long way, bool prefetch, champsim::address evicted_addr, uint32_t metadata_in)
{
  static const bool berti_norf = (std::getenv("BERTI_NO_REGION_FILTER") != nullptr);
  if (!berti_norf) {
    add_region_history(addr);
    if(evicted_addr != champsim::address{})
      remove_region_history(evicted_addr);
  }
  // We select the structures for every cpu
  LatencyTable* tlatencyt = latencyt;
  ShadowCache* tscache = scache;
  ...
  if(evicted_addr == champsim::address{} && way == static_cast<long>(cache_->num_ways())) {
  ...
  if (cycle != 0 && ((cache_->current_cycle() & TIME_MASK) > cycle))
    latency = (cache_->current_cycle() & TIME_MASK) - cycle;
...
void berti_plus::prefetcher_final_stats()
{
  std::cout << "BERTI " << "TO_L1: " << pf_to_l1 << " TO_L2: " << pf_to_l2;
  std::cout << " TO_L2_BC_MSHR: " << pf_to_l2_bc_mshr << std::endl;
  std::cout << "DETECTED ALIASES: " << scache->aliased_cache_hits << std::endl;
