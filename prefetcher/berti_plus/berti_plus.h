...
class berti_plus : public champsim::modules::prefetcher {
    ...
    explicit berti_plus(champsim::modules::ModuleBuilder builder);

  private:
    // The parent cache, captured at construction (dynamic state queries:
    // MSHR occupancy, queue sizes, current cycle).
