...
champsim::modules::prefetcher::register_module<pythia> pythia_register("pythia");
...
uint32_t pythia::prefetcher_cache_operate(champsim::address addr, champsim::address ip, bool cache_hit, bool useful_prefetch, access_type type,
                                          uint32_t metadata_in)
{
  ...
  for (uint32_t addr_index = 0; addr_index < pref_addr.size(); ++addr_index) {
    ...
    cache_->prefetch_line(pf_addr, true, 0);
  }

  return 0;
}

uint32_t pythia::prefetcher_cache_fill(champsim::address addr, long set, long way, bool prefetch, champsim::address evicted_addr, uint32_t metadata_in)
{
