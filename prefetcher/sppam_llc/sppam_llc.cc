...
template <typename T>
auto sppam_llc::Sppam_llc_Module::page_and_offset(T addr) -> std::pair<page, block_in_page>
{
  return std::pair{page{addr}, block_in_page{addr}};
}

std::pair<uint64_t,uint64_t> sppam_llc::Sppam_llc_Module::get_patterns(page pn, block_in_page page_offset) {
...
bool sppam_llc::Sppam_llc_Module::check_pagemap(champsim::address addr, bool prefetch)
{
...
std::pair<uint64_t,bool> sppam_llc::Sppam_llc_Module::get_prefetch_pattern(uint64_t pattern, int order, bool negative) {
  ...
  auto demand_pattern = negative && SEPARATE_NEGATIVE_TABLES ? negative_pattern_tables.at(intern_->cpu).at(order).check_hit(temp_pattern) : pattern_tables.at(intern_->cpu).at(order).check_hit(temp_pattern);
  ...
  if(demand_pattern.has_value()) {
    ...
    if(negative && SEPARATE_NEGATIVE_TABLES)
      negative_pattern_tables.at(intern_->cpu).at(order).fill(demand_pattern.value());
    else
      pattern_tables.at(intern_->cpu).at(order).fill(demand_pattern.value());
...
void sppam_llc::Sppam_llc_Module::increment_access_pattern(uint64_t pattern, uint64_t prediction, int order, bool negative) {
  auto temp_pattern = pattern_type{pattern & ((1 << (PATTERN_SIZE >> order)) - 1)};
  auto demand_pattern = negative && SEPARATE_NEGATIVE_TABLES ?  negative_pattern_tables.at(intern_->cpu).at(order).check_hit(temp_pattern) : pattern_tables.at(intern_->cpu).at(order).check_hit(temp_pattern);
  ...
  if(demand_pattern.has_value()) {
    ...
    if(negative && SEPARATE_NEGATIVE_TABLES)
      negative_pattern_tables.at(intern_->cpu).at(order).fill(temp_pattern);
    else
      pattern_tables.at(intern_->cpu).at(order).fill(temp_pattern);
  }
}

void sppam_llc::Sppam_llc_Module::initialize(CACHE* cache) {
  intern_ = cache;

  
  for(int n = 0; n < NUM_CPUS; n++) {
    std::size_t table_sets = PATTERN_TABLE_SETS;
    std::vector<champsim::msl::lru_table<pattern_type,pattern_table_indexer,pattern_table_indexer>> temp_table;
    std::vector<champsim::msl::lru_table<pattern_type,pattern_table_indexer,pattern_table_indexer>> temp_neg_table;
    for(int i = PATTERN_SIZE; i >= int(MIN_PATTERN_SIZE); i/=2) {
      temp_table.push_back(champsim::msl::lru_table<pattern_type,pattern_table_indexer,pattern_table_indexer>{table_sets,PATTERN_TABLE_WAYS});
      temp_neg_table.push_back(champsim::msl::lru_table<pattern_type,pattern_table_indexer,pattern_table_indexer>{table_sets,PATTERN_TABLE_WAYS});
      table_sets /= 2;
    }
    pattern_tables.push_back(temp_table);
    negative_pattern_tables.push_back(temp_table);
  }

  uint64_t total_state = get_state_bits();
  champsim::data::bytes total_bytes = champsim::data::bytes{(total_state/8) + 1};
  fmt::print("[{}] SPPAM_LLC Initialized. Total State: {}\n",intern_->NAME,champsim::data::kibibytes{total_bytes});
}



void sppam_llc::Sppam_llc_Module::add_to_pagemap(champsim::address addr, bool prefetch, champsim::address ip, uint64_t pf_trigger, page prev_page, bool direction) {
...
void sppam_llc::Sppam_llc_Module::remove_from_pagemap(champsim::address addr, bool prefetch) {
  auto [current_pn, page_offset] = page_and_offset(addr);
...
std::pair<sppam_llc::Sppam_llc_Module::page,bool> sppam_llc::Sppam_llc_Module::get_prev_page(uint32_t stream_id, page current_vpn) {
  //check cpt table
  stream_id = stream_id & BERTI_STREAM_ID_MASK;
  if(stream_id == 0)
    return {page{},true};

  bool direction = stream_id & (1<<BERTI_DIRECTION_BIT);
...
uint32_t sppam_llc::prefetcher_cache_operate(champsim::address addr, champsim::address ip, bool cache_hit, bool useful_prefetch, access_type type, uint32_t metadata_in)
{
  ...
  auto [prev_page, dir] = engine.get_prev_page(metadata_in,Sppam_llc_Module::page{addr});
  ...
  engine.track_llc_missrate(cache_hit);
...
uint32_t sppam_llc::prefetcher_cache_fill(champsim::address addr, long set, long way, bool prefetch, champsim::address evicted_addr, uint32_t metadata_in)
{
  ...
  //engine.track_llc_missrate(metadata_in);
  ///if(metadata_in != 0xffffffff)
  //  return bingo_plus_module::BINGO_WAS_LLC_MISS;
  

  return 0;
}

void sppam_llc::Sppam_llc_Module::track_llc_missrate(uint32_t hit) {
  if(hit) {
    llc_hit++;
  } else {
    llc_miss++;
  }
  if(llc_miss + llc_hit >= LLC_HITRATE_SAMPLE) {
    llc_missrate_index = std::min(15,int(llc_miss) >> (champsim::lg2(LLC_HITRATE_SAMPLE) - 4));
    llc_hit = 0;
    llc_miss = 0;
  }
}

void sppam_llc::Sppam_llc_Module::scrape_region(champsim::address addr) {
  ...
  if(region.has_value()) {
    ...
    int end_backward = region_backward.has_value() ? (PATTERN_SIZE * 2) : (PATTERN_SIZE * 3);
    ...
    if(region->momentum > FORWARD_MOMENTUM_MIN) {
      for(std::size_t i = start_forward; i <= end_forward; i++) {
        ...
        for(std::size_t j = PATTERN_SIZE; j >= MIN_PATTERN_SIZE; j/=2) {
          ...
          //fmt::print("Logging Extended Pattern from index {}: Access: {:b} Predicted: {:b}\n",i,curr_access_pattern, curr_predicted_pattern);
    ...
    if(DO_NEGATIVE && region->momentum < BACKWARD_MOMENTUM_MIN) {
      for(int64_t i = start_backward; i >= end_backward; i--) {
        ...
        for(std::size_t j = PATTERN_SIZE; j >= MIN_PATTERN_SIZE; j/=2) {
          ...
          //fmt::print("Logging Extended Reverse Pattern from index {}: Access: {:b} Predicted: {:b}\n",i,curr_access_pattern, curr_predicted_pattern);
...
int sppam_llc::Sppam_llc_Module::get_momentum(champsim::address addr) {
  auto [pn, page_offset] = page_and_offset(addr);
  auto region = regions.check_hit(region_type{pn,intern_->current_cycle()});
  if(region.has_value()) {
...
void sppam_llc::Sppam_llc_Module::do_prefetch(champsim::address addr, champsim::address ip, uint8_t cache_hit, bool useful_prefetch, access_type type, uint32_t metadata_in) {
  champsim::block_number block_addr{addr};
    //fmt::print("[SPPAM_LLC] Sent IP of: {}\n",ip.to<uint64_t>() & BERTI_IP_MASK);
  metadata_in = 0xffffffff;
  ...
  fmt::print("Doing prefetch for address: {} momentum: {}\n",addr,momentum);
        ...
        for(int i = PATTERN_SIZE; i >= int(MIN_PATTERN_SIZE); i/=2) {
          ...
          while(true) {
            ...
            if((intern_->current_cycle() % 128) < PREFETCH_DROP_CHANCE_USEFULNESS.at(current_usefulness)) {
              if((!use_pattern_confidence() || (global_usefulness_index < ALLOW_DROP_THRESH)) && PROB_DROP_PREFETCHES) {
                prefetches_dropped++;
                if((!in_warmup) && DO_DEBUG)
                fmt::print("\t\tDropped!\n");
                break;
            ...
            for(int j = 0; j < i; j++) {
              ...
              if(pos_prefetch_pattern & (1 << (i - 1 - j))) {
                ...
                if(!check_pagemap(champsim::address{pos_step_addr},true)) {
                  if(true) { //filter LLC prefetches
                    if(bool prefetch_success = intern_->prefetch_line(pos_step_addr, true, metadata_in); prefetch_success) {
                      if((!in_warmup) && DO_DEBUG)
                      ...
                      add_to_pagemap(pos_step_addr,true,ip,pos_prefetch_pattern);
                      pf_issued++;
                      if(pf_issued >= int(current_pf_degree))
                        break;
            ...
            for(int j = 0; j < i; j++) {
              ...
              if(neg_prefetch_pattern & (1 << (i - 1 - j))) {
                ...
                if(!check_pagemap(champsim::address{neg_step_addr},true)) {
                  if(true) {
                    if(bool prefetch_success = intern_->prefetch_line(neg_step_addr, true, metadata_in); prefetch_success) {
                      //fmt::print("AMPM Prefetched: {} Metadata: {}\n",neg_step_addr,metadata_in);
                      add_to_pagemap(neg_step_addr,true,ip,neg_prefetch_pattern);
                      prefetches_issued++; 
                      pf_issued++;
                      if(pf_issued >= int(current_pf_degree))
                        break;
...
void sppam_llc::Sppam_llc_Module::print_patterns() {
    ...
    for(uint64_t j = PATTERN_SIZE; j >= int(MIN_PATTERN_SIZE); j/=2) {
      ...
      if(TABLE_OR_COUNTER) {
        for(uint64_t i = 0; i < (1ul<<j); i++) {
          auto was_hit = pattern_tables.at(n).at(order).check_hit(pattern_type{i});
          auto was_hit_neg = negative_pattern_tables.at(n).at(order).check_hit(pattern_type{i});
          if(was_hit.has_value()) {
            fmt::print("Pattern: {:b} Triggers: {} Usefulness: {}\n",i, was_hit->occurrences, was_hit->usefulness * (100/16.0));
          ...
          if(was_hit_neg.has_value()) {
            fmt::print("Negative Pattern: {:b} Triggers: {} Usefulness: {}\n",i, was_hit_neg->occurrences, was_hit_neg->usefulness * (100/16.0));
            for(int64_t len = j-1; len >= 0; len--) {
...
uint64_t sppam_llc::Sppam_llc_Module::get_state_bits() {
  //Assuming 48-bit max address
  //calculate total bits needed for Sppam_llc_Module
  uint64_t total_bits = 0;

  //start with region table
  uint64_t region_table_bits = (48 - champsim::lg2(SPPAM_LLC_PAGE_BITS)) * 3; //page nums
  region_table_bits += ((1 << SPPAM_LLC_PAGE_BITS) / BLOCK_SIZE)*2; //bitmaps
  region_table_bits += SCRAPE_ON_IDLE ? champsim::lg2(SCRAPE_IDLE_TIME) : 0; //scrape count
  region_table_bits += REGION_PATTERN_TAG ? (((1 << SPPAM_LLC_PAGE_BITS) / BLOCK_SIZE) * PATTERN_SIZE) : 0;
  //region_table_bits += 4 //momentum (DISABLED)
  //region_table_bits += champsim::lg2(((1 << SPPAM_LLC_PAGE_BITS) / BLOCK_SIZE)); //last block
  ...
  //pattern table
  uint64_t pattern_table_bits = PATTERN_SIZE; //tag
  ...
  pattern_table_bits *= SEPARATE_NEGATIVE_TABLES ? 2 : 1; //doubled if separate tables
  pattern_table_bits *= NUM_CPUS;
  ...
  misc_state += (ADAPTIVE_USEFULNESS || !GLOBAL_OR_PATTERN_USEFULNESS) ? (champsim::lg2(GLOBAL_USEFULNESS_SAMPLE)*2 + 4) : 0; //global usefulness counter
  misc_state += (champsim::lg2(REGION_LIFESPAN_SAMPLE) * 2) + 4; //lifespan sampler
  misc_state += (champsim::lg2(LLC_HITRATE_SAMPLE) * 2) + 4;
  misc_state += 4 + 4; //lookahead usefulness reg, bandwidth reg


  //total state tally
  total_bits += region_table_bits + pattern_table_bits + cpt_bits + misc_state;


  return total_bits;
}

bool sppam_llc::Sppam_llc_Module::use_pattern_confidence() {
  //static 
...
void sppam_llc::Sppam_llc_Module::modify_pattern_usefulness(champsim::address addr, bool useful) {
  auto [pn, page_offset] = page_and_offset(addr);
  auto region = regions.check_hit(region_type{pn,intern_->current_cycle()});
  if(!region.has_value()) {
    if(!useful)
    ...
    for(int i = PATTERN_SIZE; i >= int(MIN_PATTERN_SIZE); i/=2) {
      ...
      for(int o = 1; o <= i; o++) {
        ...
        if(region->momentum > FORWARD_MOMENTUM_MIN) {
          ...
          auto demand_pattern = pattern_tables.at(intern_->cpu).at(order).check_hit(temp_pattern);
        ...
        if(region->momentum < BACKWARD_MOMENTUM_MIN) {
          ...
          if(pattern == 0)
            continue;
          auto temp_pattern = pattern_type{pattern & ((1 << (i)) - 1)};;
          auto demand_pattern = negative_pattern_tables.at(intern_->cpu).at(order).check_hit(temp_pattern);
          if(demand_pattern.has_value()) {
            if(!use_pattern_confidence()) {
              demand_pattern->usefulness = global_usefulness_index;
            }
...
void sppam_llc::prefetcher_cycle_operate() {
  ...
  if(!intern_->warmup && engine.in_warmup) {
    fmt::print("[{}] SPPAM_LLC Resetting internal stats\n",intern_->NAME);
...
int sppam_llc::Sppam_llc_Module::set_prefetch_degree(uint64_t pattern, int order, bool negative, int prev_usefulness = 15) {
  ...
  if((GLOBAL_OR_PATTERN_USEFULNESS && !ADAPTIVE_USEFULNESS) || (use_pattern_confidence() && ADAPTIVE_USEFULNESS)) {
    auto demand_pattern = (negative && SEPARATE_NEGATIVE_TABLES) ? negative_pattern_tables.at(intern_->cpu).at(order).check_hit(pattern_type{pattern & ((1 << (PATTERN_SIZE >> order)) - 1)}) : pattern_tables.at(intern_->cpu).at(order).check_hit(pattern_type{pattern & ((1 << (PATTERN_SIZE >> order)) - 1)});
    if(demand_pattern.has_value()) {
...
void sppam_llc::Sppam_llc_Module::decrease_usefulness_counter() {
  /*
  ...
  */
  ...
  if(global_useful_prefetch + global_useless_prefetch >= GLOBAL_USEFULNESS_SAMPLE) {
    global_usefulness_index = std::min(15,int(global_useful_prefetch) >> (champsim::lg2(GLOBAL_USEFULNESS_SAMPLE) - 4));
    global_useful_prefetch = 0;
    global_useless_prefetch = 0;
  }
}

void sppam_llc::Sppam_llc_Module::increase_usefulness_counter() {
  /*
...
int sppam_llc::Sppam_llc_Module::do_4_bit_mult(int op1, int op2) {
...
void sppam_llc::prefetcher_final_stats() {
  ...
  fmt::print("\tRegions Scraped: {} Shadow-filled fronts: {} Shadow-filled backs: {}\n",engine.regions_scraped,engine.shadow_scrapes_used_backward,engine.shadow_scrapes_used_forward);
