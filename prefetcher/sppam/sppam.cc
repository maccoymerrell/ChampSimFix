...
// Register the model under the config name "sppam".
champsim::modules::prefetcher::register_module<sppam> sppam_register("sppam");

// DPC4 exposed instantaneous DRAM bandwidth through the global get_dram_bw().
// The runtime has no equivalent reachable from a prefetcher module (matching the
// pythia runtime port, which stubs the same call). Return 0 (the neutral,
// lowest-bandwidth quantum): the subtract-mode bandwidth throttle then subtracts
// nothing, so the prefetcher's degree is governed purely by usefulness, exactly
// as in the unloaded DPC4 case.
static uint8_t get_dram_bw() { return 0; }

sppam::sppam(champsim::modules::ModuleBuilder builder)
    : cache_(builder.get_parent<champsim::modules::cache_module>())
{
  intern_ = dynamic_cast<CACHE*>(cache_);
  engine.NUM_CPUS =
      static_cast<uint32_t>(champsim::modules::ModuleBuilder::globals().get_parameter<std::size_t>("num_consumers", true, std::size_t{1}));
}
...
void sppam::prefetcher_cycle_operate() {
  ...
  if(!intern_->is_warmup() && engine.in_warmup) {
