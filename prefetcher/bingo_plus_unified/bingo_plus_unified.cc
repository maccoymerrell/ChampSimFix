...
#include "bingo_plus_unified.h"

using namespace Bingo_plus_unified;
 ...
 */ 
uint64_t hash_index_plus_unified(uint64_t key, int index_len) {
    if (index_len == 0)
        return key;
...
void FilterTable::write_data(Entry<FilterTableData> &entry, Table &table, int row) {
    uint64_t key = hash_index_plus_unified(entry.key, this->index_len);
...
uint64_t FilterTable::build_key(uint64_t region_number) {
    ...
    return hash_index_plus_unified(key, this->index_len);
}
...
void PatternHistoryTable::write_data(Entry<PatternHistoryTableData> &entry, Table &table, int row) {
    ...
    index_key = hash_index_plus_unified(index_key, this->index_len); /* unhash */
...
uint64_t PatternHistoryTable::build_key(uint64_t pc, uint64_t address) {
    ...
    uint64_t index_key = hash_index_plus_unified((pc << this->min_addr_width) | offset, this->index_len);
...
int PrefetchStreamer::prefetch(bingo_plus_unified_module *b, uint64_t block_address) {
    if (this->debug_level >= 2) {
        ...
        for (int sgn = +1; sgn >= -1; sgn -= 2) {
            ...
            if (0 <= pf_offset && pf_offset < this->pattern_len && pattern[pf_offset] > 0) {
                ...
                if (b->intern_->get_pq_occupancy().back() + b->intern_->get_mshr_occupancy() < b->intern_->get_mshr_size() - 1 &&
                    ...
                    int ok = b->module_->prefetch_line(pf_address, pattern[pf_offset], 0);
                    // assert(ok == 1);
                    pf_issued += 1;
                    pattern[pf_offset] = 0;
                } else {
...
uint64_t PrefetchStreamer::build_key(uint64_t region_number) { return hash_index_plus_unified(region_number, this->index_len); }


template <class T> inline T square(T x) { return x * x; }

void bingo_plus_unified_module::access(uint64_t block_number, uint64_t pc) {
    if (this->debug_level >= 2)
    ...
    if (entry->data.offset != region_offset) {
        ...
        region_number = hash_index_plus_unified(entry->key, this->filter_table.get_index_len());
...
void bingo_plus_unified_module::eviction(uint64_t block_number) {
    if (this->debug_level >= 2)
...
int bingo_plus_unified_module::prefetch(uint64_t block_number) {
    if (this->debug_level >= 2)
...
void bingo_plus_unified_module::set_debug_level(int debug_level_) {
...
void bingo_plus_unified_module::log() {
...
Event bingo_plus_unified_module::get_event(uint64_t block_number) {
    uint64_t region_number = block_number / this->pattern_len;
    // assert(this->pht_events.count(region_number) == 1);
    return this->pht_events[region_number];
}

void bingo_plus_unified_module::add_prefetch(uint64_t block_number) {
    Event ev = this->get_event(block_number);
    // assert(ev != MISS);
    this->prefetch_cnt[ev] += 1;
}

void bingo_plus_unified_module::add_useful(uint64_t block_number, Event ev) {
    // assert(ev != MISS);
    this->useful_cnt[ev] += 1;
}

void bingo_plus_unified_module::add_useless(uint64_t block_number, Event ev) {
    // assert(ev != MISS);
    this->useless_cnt[ev] += 1;
}

void bingo_plus_unified_module::reset_stats() {
...
void bingo_plus_unified_module::print_stats() {
 ...
 */
std::vector<bool> bingo_plus_unified_module::find_in_pht(uint64_t pc, uint64_t address) {
    if (this->debug_level >= 2) {
...
void bingo_plus_unified_module::insert_in_pht(const Entry<AccumulationTableData> &entry) {
    uint64_t pc = entry.data.pc;
    uint64_t region_number = hash_index_plus_unified(entry.key, this->accumulation_table.get_index_len());
 ...
 */
std::vector<bool> bingo_plus_unified_module::vote(const std::vector<std::vector<bool>> &x) {
    if (this->debug_level >= 2)
...
void bingo_plus_unified::prefetcher_initialize() {

    uint64_t bingo_state = 249624;
    auto bingo_bytes = champsim::data::bytes{bingo_state};
    prefetchers.emplace_back(intern_,this,0);
    
    fmt::print("[{}] Bingo+ Initialized. Total State: {}\n",intern_->NAME,champsim::data::kibibytes{bingo_bytes});
    /*if (bingo_plus_unified_module::DEBUG_LEVEL >= 1)
        std::cerr << "Bingo::Bingo(pattern_len=" << pattern_len << ", min_addr_width=" << bingo_plus_unified_module::MIN_ADDR_WIDTH
                << ", max_addr_width=" << bingo_plus_unified_module::MAX_ADDR_WIDTH << ", pc_width=" << bingo_plus_unified_module::PC_WIDTH
                << ", filter_table_size=" << bingo_plus_unified_module::FT_SIZE
                << ", accumulation_table_size=" << bingo_plus_unified_module::AT_SIZE << ", pht_size=" << bingo_plus_unified_module::PHT_SIZE
                << ", pht_ways=" << bingo_plus_unified_module::PHT_WAYS << ", pf_streamer_size=" << bingo_plus_unified_module::PF_STREAMER_SIZE
                << ", debug_level=" << bingo_plus_unified_module::DEBUG_LEVEL << ")" << std::endl;*/
}

uint32_t bingo_plus_unified::prefetcher_cache_operate(champsim::address addr, champsim::address ip, bool cache_hit, bool useful_prefetch, access_type type, uint32_t metadata_in)
{
    if (bingo_plus_unified_module::DEBUG_LEVEL >= 2) {
    ...
    prefetchers[0].access(block_number.to<uint64_t>(), ip.to<uint64_t>());

    /* issue prefetches */
    prefetchers[0].prefetch(block_number.to<uint64_t>());

    if (bingo_plus_unified_module::DEBUG_LEVEL >= 3) {
        prefetchers[0].log();
...
uint32_t bingo_plus_unified::prefetcher_cache_fill(champsim::address addr, long set, long way, bool prefetch, champsim::address evicted_addr, uint32_t metadata_in) {
    ...
    prefetchers[0].eviction(evicted_block_number.to<uint64_t>());

    return metadata_in;
}

void bingo_plus_unified::prefetcher_final_stats() {
    prefetchers[0].print_stats();
}
void bingo_plus_unified::prefetcher_cycle_operate() {}