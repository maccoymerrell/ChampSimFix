#ifndef PREFETCHER_BINGO_PLUS_UNIFIED_H
#define PREFETCHER_BINGO_PLUS_UNIFIED_H
...
class bingo_plus_unified_module;

namespace Bingo_plus_unified{
...
class PrefetchStreamer : public LRUSetAssociativeCache<PrefetchStreamerData> {
  ...
  public:
    ...
    int prefetch(bingo_plus_unified_module *b, uint64_t block_address);
    std::string log();
...
class bingo_plus_unified_module {
  public:
    ...
    static constexpr int PHT_SIZE = 32768;     /* size of pattern history table (PHT) */
    static constexpr int PHT_WAYS = 32;           /* associativity of PHT */
    ...
    Bingo_plus_unified::Event get_event(uint64_t block_number);
    void add_prefetch(uint64_t block_number);
    void add_useful(uint64_t block_number, Bingo_plus_unified::Event ev);
    void add_useless(uint64_t block_number, Bingo_plus_unified::Event ev);
    void reset_stats();
    void print_stats();
  ...
  private:
    ...
    void insert_in_pht(const Bingo_plus_unified::Entry<Bingo_plus_unified::AccumulationTableData> &entry);
    ...
    Bingo_plus_unified::FilterTable filter_table{FT_SIZE,DEBUG_LEVEL};
    Bingo_plus_unified::AccumulationTable accumulation_table{AT_SIZE, REGION_SIZE >> LOG2_BLOCK_SIZE, DEBUG_LEVEL};
    Bingo_plus_unified::PatternHistoryTable pht{PHT_SIZE,REGION_SIZE >> LOG2_BLOCK_SIZE, MIN_ADDR_WIDTH, MAX_ADDR_WIDTH, PC_WIDTH, DEBUG_LEVEL, PHT_WAYS};
    Bingo_plus_unified::PrefetchStreamer pf_streamer{PF_STREAMER_SIZE, REGION_SIZE >> LOG2_BLOCK_SIZE, DEBUG_LEVEL};
    int debug_level = DEBUG_LEVEL;

    /* stats */
    std::unordered_map<uint64_t, Bingo_plus_unified::Event> pht_events;
    ...
    public:
      bingo_plus_unified_module(CACHE* intern, champsim::modules::prefetcher* module__, int cpu_) : intern_(intern), module_(module__), cpu(cpu_) {}
};

class bingo_plus_unified : public champsim::modules::prefetcher {
    std::vector<bingo_plus_unified_module> prefetchers;
    int pattern_len = bingo_plus_unified_module::REGION_SIZE >> LOG2_BLOCK_SIZE;
