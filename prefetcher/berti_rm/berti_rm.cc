...
void berti_rm::prefetcher_initialize()
{
  std::cout << " L1D Berti_rm prefetcher" << std::endl;
...
uint32_t berti_rm::prefetcher_cache_operate(champsim::address address, champsim::address ip_addr, uint8_t cache_hit, bool useful_prefetch, access_type type,
                                         uint32_t metadata_in)
{
  ...
  uint64_t page_addr = line_addr >> L1D_RMPAGE_BLOCKS_BITS;
  uint64_t offset = line_addr & L1D_RMPAGE_OFFSET_MASK;
  ...
  if (index == L1D_RMCURRENT_PAGES_TABLE_ENTRIES || !l1d_requested_offset_current_pages_table(index, offset)) {
    // cout << "OFFSETS: " << hex << ip << " " << page_addr << " " << dec << offset << endl;

    if (index < L1D_RMCURRENT_PAGES_TABLE_ENTRIES) { // Found
      // cout << " FOUND" << endl;
      ...
      assert(l1d_ip_table[first_ip & L1D_RMIP_TABLE_INDEX_MASK] != L1D_RMIP_TABLE_NULL_POINTER);

      // Update berti_rm
      if (cache_hit) {
        uint64_t pref_latency = l1d_get_latency_prev_prefetches_table(index, offset);
        if (pref_latency != 0) {
          // Find berti_rm distance from pref_latency cycles before
          int b[L1D_RMCURRENT_PAGES_TABLE_NUM_BERTI_RM_PER_ACCESS];
          l1d_get_berti_rm_prev_requests_table(index, offset, current_core_cycle - pref_latency, b);
          for (int i = 0; i < L1D_RMCURRENT_PAGES_TABLE_NUM_BERTI_RM_PER_ACCESS; i++) {
            if (b[i] == 0)
              break;
            assert(abs(b[i]) < L1D_RMPAGE_BLOCKS);
            l1d_add_berti_rm_current_pages_table(index, b[i]);
          }
      ...
      if (first_ip != ip) {
        ...
        l1d_ip_table[ip & L1D_RMIP_TABLE_INDEX_MASK] = l1d_ip_table[first_ip & L1D_RMIP_TABLE_INDEX_MASK];
      }
      ...
      assert(victim_index < L1D_RMCURRENT_PAGES_TABLE_ENTRIES);
      ...
      index = victim_index;
      l1d_add_current_pages_table(index, page_addr, ip & L1D_RMIP_TABLE_INDEX_MASK, offset);

      // Set pointer in IP table
      uint64_t index_record = l1d_get_entry_record_pages_table(page_addr, offset);
      // The ip pointer is null
      if (l1d_ip_table[ip & L1D_RMIP_TABLE_INDEX_MASK] == L1D_RMIP_TABLE_NULL_POINTER) {
        if (index_record == L1D_RMRECORD_PAGES_TABLE_ENTRIES) { // Page not recorded
          // Get free record page pointer.
          uint64_t new_pointer = l1d_get_lru_record_pages_entry();
          l1d_ip_table[ip & L1D_RMIP_TABLE_INDEX_MASK] = new_pointer;
        } else {
          l1d_ip_table[ip & L1D_RMIP_TABLE_INDEX_MASK] = index_record;
        }
      } else if (l1d_ip_table[ip & L1D_RMIP_TABLE_INDEX_MASK] != index_record) {
        ...
        uint64_t new_pointer = l1d_get_lru_record_pages_entry();
        l1d_copy_entries_record_pages_table(l1d_ip_table[ip & L1D_RMIP_TABLE_INDEX_MASK], new_pointer);
        l1d_ip_table[ip & L1D_RMIP_TABLE_INDEX_MASK] = new_pointer;
      }
    }
    ...
    uint64_t ip_pointer = l1d_ip_table[ip & L1D_RMIP_TABLE_INDEX_MASK];
    ...
    uint64_t berti_rm_confidence = 0;
    int current_berti_rm = l1d_get_berti_rm_current_pages_table(index, berti_rm_confidence);
    uint64_t match_confidence = 0;

    // If match with current page+first_offset, use record
    if (pgo_pointer != L1D_RMRECORD_PAGES_TABLE_ENTRIES
      ...
      b = l1d_record_pages_table[pgo_pointer].berti_rm;
      ...
      if (l1d_record_pages_table[ip_pointer].first_offset == first_offset
        ...
        b = l1d_record_pages_table[ip_pointer].berti_rm;
        match_confidence = 1; // High confidence
        recorded = true;
      } else
        // If no exact match, trust current if it has already a berti_rm
        if (current_berti_rm != 0 && berti_rm_confidence >= L1D_RMBERTI_RM_CTR_MED_HIGH_CONFIDENCE) { // Medium-High confidence
          u_vector = l1d_current_pages_table[index].u_vector;
          b = current_berti_rm;
        } else
          // If match with current page, use record
          if (pg_pointer != L1D_RMRECORD_PAGES_TABLE_ENTRIES) { // Medium confidence
            u_vector = l1d_record_pages_table[pg_pointer].u_vector;
            b = l1d_record_pages_table[pg_pointer].berti_rm;
            recorded = true;
          } else
      ...
      if (recorded && match_confidence) {
        ...
        if (b > 0) {
          for (uint64_t i = first_burst; i < offset + b; i++) {
            if ((int)i >= L1D_RMPAGE_BLOCKS)
              break; // Stay in the page
            // Only if previously requested and not demanded
            uint64_t pf_line_addr = (page_addr << L1D_RMPAGE_BLOCKS_BITS) | i;
            uint64_t pf_addr = pf_line_addr << LOG2_BLOCK_SIZE;
            uint64_t pf_page_addr = pf_line_addr >> L1D_RMPAGE_BLOCKS_BITS;
            uint64_t pf_offset = pf_line_addr & L1D_RMPAGE_OFFSET_MASK;
            if ((((uint64_t)1 << i) & u_vector) && !l1d_requested_offset_current_pages_table(index, pf_offset)) {
              if (pq_occupancy < pq_size && bursts < L1D_RMMAX_NUM_BURST_PREFETCHES) {
                if ((intern_->virtual_prefetch
                    || pf_page_addr == page_addr) && !get_region_history(champsim::address{pf_addr})) { // either Berti_rm is prefetching in virtual addr space, or it needs to obey page boundary
        ...
        } else { // berti_rm == 0 (zig zag of all)
          for (int i = (int)first_burst, j = (int)((first_offset << 1) - i); i < L1D_RMPAGE_BLOCKS || j >= 0; i++, j = (int)((first_offset << 1) - i)) {
            // Only if previously requested and not demanded
            // Dir ++
            uint64_t pf_line_addr = (page_addr << L1D_RMPAGE_BLOCKS_BITS) | i;
            uint64_t pf_addr = pf_line_addr << LOG2_BLOCK_SIZE;
            ...
            // Dir --
            pf_line_addr = (page_addr << L1D_RMPAGE_BLOCKS_BITS) | j;
            pf_addr = pf_line_addr << LOG2_BLOCK_SIZE;
            pf_page_addr = pf_line_addr >> L1D_RMPAGE_BLOCKS_BITS;
            pf_offset = pf_line_addr & L1D_RMPAGE_OFFSET_MASK;
...
uint32_t berti_rm::prefetcher_cache_fill(champsim::address address, long set, long way, uint8_t prefetch, champsim::address evicted_address, uint32_t metadata_in)
{
  add_region_history(address);
  if(evicted_address != champsim::address{})
    remove_region_history(evicted_address);
  uint64_t addr = address.to<uint64_t>();
  ...
  if (pointer_prev < L1D_RMCURRENT_PAGES_TABLE_ENTRIES) { // Not found, not entry in prev requests
    ...
    if (demand_latency != 0) { // Not found, berti_rm will not be found neither

      // Find berti_rm (distance from pref_latency + demand_latency cycles before
      int b[L1D_RMCURRENT_PAGES_TABLE_NUM_BERTI_RM_PER_ACCESS];
      l1d_get_berti_rm_prev_requests_table(pointer_prev, offset, current_core_cycle - (pref_latency + demand_latency), b);
      for (int i = 0; i < L1D_RMCURRENT_PAGES_TABLE_NUM_BERTI_RM_PER_ACCESS; i++) {
        if (b[i] == 0)
          break;
        assert(abs(b[i]) < L1D_RMPAGE_BLOCKS);
        l1d_add_berti_rm_current_pages_table(pointer_prev, b[i]);
      }
    }
  }

  uint64_t victim_index = l1d_get_current_pages_entry(evicted_addr >> LOG2_PAGE_SIZE);
  if (victim_index < L1D_RMCURRENT_PAGES_TABLE_ENTRIES) {
    // Copy victim to record table
    l1d_record_current_page(victim_index);
...
bool berti_rm::get_region_history(champsim::address addr) {
...
void berti_rm::remove_region_history(champsim::address addr) {
  region_type::page pn = region_type::page(addr);
  region_type::block_in_page po = region_type::block_in_page{addr};
  auto temp_region = region_type{pn};
...
void berti_rm::add_region_history(champsim::address addr) {
  region_type::page pn = region_type::page(addr);
  region_type::block_in_page po = region_type::block_in_page{addr};
  auto temp_region = region_type{pn};
...
void berti_rm::prefetcher_cycle_operate() {}