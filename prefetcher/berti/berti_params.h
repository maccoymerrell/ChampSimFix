// ChampSim's modular build no longer exposes global LOG2_BLOCK_SIZE /
// LOG2_PAGE_SIZE macros (modules read block/page size from the config).
// Berti uses these in compile-time macro contexts, so define them here for the
// standard 64-byte cache line / 4KB page used by all DPC4 traces.
