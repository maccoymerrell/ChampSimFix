...
champsim::modules::prefetcher::register_module<berti> berti_register("berti");

berti::berti(champsim::modules::ModuleBuilder builder) : cache_(builder.get_parent<champsim::modules::cache_module>()) {}
...
uint32_t berti::prefetcher_cache_operate(champsim::address address, champsim::address ip_addr, bool cache_hit, bool useful_prefetch, access_type type,
                                         uint32_t metadata_in)
{
  ...
  auto current_core_cycle = cache_->current_time.time_since_epoch() / cache_->clock_period;

  uint64_t line_addr = addr >> LOG2_BLOCK_SIZE;
  uint64_t page_addr = line_addr >> L1D_PAGE_BLOCKS_BITS;
  uint64_t offset = line_addr & L1D_PAGE_OFFSET_MASK;

  std::size_t pq_size = cache_->get_pq_size()[0];
  std::size_t pq_occupancy = cache_->get_pq_occupancy()[0];
      ...
      if (recorded && match_confidence) {
        ...
        if (b > 0) {
            ...
            if ((((uint64_t)1 << i) & u_vector) && !l1d_requested_offset_current_pages_table(index, pf_offset)) {
              if (pq_occupancy < pq_size && bursts < L1D_MAX_NUM_BURST_PREFETCHES) {
                if (cache_->is_virtual_prefetch()
                  ...
                  bool prefetched = prefetch_line(champsim::address{pf_addr}, true, 0);
                  if (prefetched) {
          ...
          for (int i = (int)first_burst; i > ((int)offset) + b; i--) {
            ...
            if ((((uint64_t)1 << i) & u_vector) && !l1d_requested_offset_current_pages_table(index, pf_offset)) {
              if (pq_occupancy < pq_size && bursts < L1D_MAX_NUM_BURST_PREFETCHES) {
                if (cache_->is_virtual_prefetch() || pf_page_addr == page_addr) {
                  bool prefetched = prefetch_line(champsim::address{pf_addr}, true, 0);
                  if (prefetched) {
                    l1d_add_prev_prefetches_table(index, pf_offset, current_core_cycle);
                    bursts++;
...
uint32_t berti::prefetcher_cache_fill(champsim::address address, long set, long way, bool prefetch, champsim::address evicted_address, uint32_t metadata_in)
{
  uint64_t addr = address.to<uint64_t>();
