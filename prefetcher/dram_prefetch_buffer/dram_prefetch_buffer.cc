#include "dram_prefetch_buffer.h"

uint32_t dram_prefetch_buffer::prefetcher_cache_operate(champsim::address addr, champsim::address ip, uint32_t cpu, uint8_t cache_hit, bool useful_prefetch, access_type type,
...
uint8_t dram_prefetch_buffer::get_confidence(champsim::address addr) {
...
void dram_prefetch_buffer::update_row_interval_table(champsim::address addr) {
  std::size_t rb = MEMORY_CONTROLLER::DRAM_CONTROLLER.value()->dram_get_rowbuffer(addr);
  row_interval_table[rb].activations++;
}

void dram_prefetch_buffer::update_row_open_table(champsim::address addr) {
  std::size_t rb = MEMORY_CONTROLLER::DRAM_CONTROLLER.value()->dram_get_rowbuffer(addr);
  std::size_t row = MEMORY_CONTROLLER::DRAM_CONTROLLER.value()->dram_get_row(addr);
...
bool dram_prefetch_buffer::is_row_open(champsim::address addr) {
  std::size_t rb = MEMORY_CONTROLLER::DRAM_CONTROLLER.value()->dram_get_rowbuffer(addr);
  std::size_t row = MEMORY_CONTROLLER::DRAM_CONTROLLER.value()->dram_get_row(addr);
  return row_open_table[rb].row == row;
}

void dram_prefetch_buffer::update_row_confidence(champsim::address addr) {
...
void dram_prefetch_buffer::increase_confidence(champsim::address addr, uint8_t amnt, bool cond) {
  std::size_t rb = MEMORY_CONTROLLER::DRAM_CONTROLLER.value()->dram_get_rowbuffer(addr);
  std::size_t row = MEMORY_CONTROLLER::DRAM_CONTROLLER.value()->dram_get_row(addr);
  std::size_t col = MEMORY_CONTROLLER::DRAM_CONTROLLER.value()->dram_get_column(addr);
...
void dram_prefetch_buffer::decrease_confidence(champsim::address addr, uint8_t amnt) {
  std::size_t rb = MEMORY_CONTROLLER::DRAM_CONTROLLER.value()->dram_get_rowbuffer(addr);
  std::size_t row = MEMORY_CONTROLLER::DRAM_CONTROLLER.value()->dram_get_row(addr);
  std::size_t col = MEMORY_CONTROLLER::DRAM_CONTROLLER.value()->dram_get_column(addr);
...
std::size_t dram_prefetch_buffer::get_depth(uint8_t conf) {
  uint8_t threshold = 0;
...
bool dram_prefetch_buffer::add_to_pf_issuer(champsim::address addr, std::size_t depth, uint32_t cpu) {
...
void dram_prefetch_buffer::do_pf_issue() {
...
void dram_prefetch_buffer::update_walker(champsim::address addr, uint32_t cpu) {
  std::size_t rb = MEMORY_CONTROLLER::DRAM_CONTROLLER.value()->dram_get_rowbuffer(addr);
  std::size_t row = MEMORY_CONTROLLER::DRAM_CONTROLLER.value()->dram_get_row(addr);
  std::size_t col = MEMORY_CONTROLLER::DRAM_CONTROLLER.value()->dram_get_column(addr);
...
void dram_prefetch_buffer::issue_row_prefetch(champsim::address addr, uint32_t cpu) {
  std::size_t rb = MEMORY_CONTROLLER::DRAM_CONTROLLER.value()->dram_get_rowbuffer(addr);
  std::size_t row = MEMORY_CONTROLLER::DRAM_CONTROLLER.value()->dram_get_row(addr);
  std::size_t col = MEMORY_CONTROLLER::DRAM_CONTROLLER.value()->dram_get_column(addr);
...
champsim::address dram_prefetch_buffer::compose_base_and_column(champsim::address base, uint64_t column) {
...
void dram_prefetch_buffer::prefetcher_initialize() {
