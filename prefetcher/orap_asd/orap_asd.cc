...
uint32_t orap_asd::prefetcher_cache_operate(champsim::address addr, champsim::address ip, uint32_t cpu, uint8_t cache_hit, bool useful_prefetch, access_type type,
                                             uint32_t metadata_in, uint32_t metadata_hit)
{
  ...
  if(metadata_in != ASD_ID && !DISABLE_ASD) {
    double asd_thresh = get_asd_thresh(get_confidence_from_ip_hash(get_hash(ip.to<uint64_t>()),cpu,true),cpu);
    ASD_Modules.at(cpu).set_histogram_factor(asd_thresh);
    ...
    if(USE_PREFETCH_QUEUE) {
      if(depth != 0) {
        champsim::address pf_addr = champsim::address{champsim::block_number{addr} + stride};
        add_to_pq(prefetch_queue_entry(pf_addr,
                                      depth,
                                      stride,
                                      ip,
                                      cpu,
...
bool orap_asd::update_row_open_table(champsim::address addr) {
...
uint8_t orap_asd::modify_confidence(uint8_t conf, uint8_t amnt, bool increment) {
  if(increment) {
...
uint8_t orap_asd::modify_confidence(uint8_t conf, double factor) {
...
void orap_asd::increase_confidence_useful(champsim::address ip, champsim::address addr, uint32_t cpu, bool coprefetch) {
...
void orap_asd::increase_confidence_opened(champsim::address ip, champsim::address addr, uint32_t cpu, bool coprefetch) {
  //fmt::print("Increasing confidence, row: {} rb: {} col: {}\n",row,rb,col);
...
void orap_asd::decrease_confidence_useless(champsim::address addr, uint32_t cpu, bool coprefetch) {
  std::size_t row = MEMORY_CONTROLLER::DRAM_CONTROLLER.value()->dram_get_row(addr);
...
void orap_asd::decrease_confidence_conflict(champsim::address ip, champsim::address addr, uint32_t cpu, bool coprefetch) {
  std::size_t row = MEMORY_CONTROLLER::DRAM_CONTROLLER.value()->dram_get_row(addr);
  /*
  uint16_t ip_hash = get_hash(ip.to<uint64_t>());
  ip_tracker it{ip_hash};
...
void orap_asd::decrease_confidence_fill(champsim::address ip, champsim::address addr, uint32_t cpu, bool coprefetch) {
  std::size_t row = MEMORY_CONTROLLER::DRAM_CONTROLLER.value()->dram_get_row(addr);
  uint16_t ip_hash = get_hash(ip.to<uint64_t>());
  ip_tracker it{ip_hash};
...
std::size_t orap_asd::get_depth(uint8_t conf, uint32_t cpu) {
  uint8_t threshold = 0;
...
uint8_t orap_asd::get_squash_chance(uint8_t conf, uint32_t cpu) {
  uint8_t threshold = 0;
  uint8_t new_conf = conf < variable_buffer_conf[cpu] ? 0 : conf - variable_buffer_conf[cpu];
  for (auto thres : THRESH) {
    threshold++;
...
double orap_asd::get_asd_thresh(uint8_t conf, uint32_t cpu) {
  uint8_t threshold = 0;
  for (auto thres : THRESH) {
    threshold++;
    if(conf < thres)
...
void orap_asd::update_walker(champsim::address addr, uint32_t cpu, champsim::address ip, bool is_blacklisted) {
  std::size_t row = MEMORY_CONTROLLER::DRAM_CONTROLLER.value()->dram_get_row(addr);
...
champsim::address orap_asd::compose_base_and_column(champsim::address base, uint64_t column) {
...
uint32_t orap_asd::prefetcher_cache_fill(champsim::address addr, champsim::address ip, uint32_t cpu, bool useless, long set, long way, uint8_t prefetch, champsim::address evicted_addr, uint32_t metadata_in, uint32_t metadata_evict, uint32_t cpu_evict)
{
  std::size_t row = MEMORY_CONTROLLER::DRAM_CONTROLLER.value()->dram_get_row(addr);
...
void orap_asd::prefetcher_cycle_operate() {
  epoch_cycle_counter++;
...
void orap_asd::prefetcher_final_stats() {
...
void orap_asd::reset_ip_blacklist() {
  for(int i = 0; i < NUM_CPUS; i++) {
    ip_blacklist_table[i].flush();
  }
}

bool orap_asd::is_ip_blacklisted(champsim::address ip, uint32_t cpu) {
...
void orap_asd::update_ip_blacklist(std::size_t collision_type, champsim::address ip, uint32_t cpu, uint32_t victim_cpu, uint32_t target_prefetcher, uint32_t victim_prefetcher) {
  ip_blacklist_counter bc{ip};
...
std::vector<uint16_t> orap_asd::get_ip_hash_from_row(uint32_t row, uint32_t cpu) {
  row_walker rw{row};
...
uint8_t orap_asd::get_confidence_from_ip_hash(uint16_t ip_hash, uint32_t cpu, bool coprefetch) {
  ip_tracker it{ip_hash};
...
uint8_t orap_asd::get_confidence_from_row(champsim::address addr, uint32_t cpu) {
  std::size_t row = MEMORY_CONTROLLER::DRAM_CONTROLLER.value()->dram_get_row(addr);
  row_walker rw{row};
...
void orap_asd::log_ip_hash_to_row(uint32_t row, uint16_t ip_hash, uint32_t cpu) {
  row_walker rw{row};
...
void orap_asd::add_to_pq(prefetch_queue_entry pqe) {
    ...
    for(int i = 0; i < pqe.length; i++) {
      ...
      if(ASD_Modules.at(pqe.cpu).check_pagemap(temp_pqe.start_addr) || champsim::page_number{temp_pqe.start_addr} != champsim::page_number{pqe.start_addr}) {
        if(!intern_->warmup)
          prefetches_filtered++;
        continue;
      }
...
void orap_asd::clear_from_pq(champsim::address addr, uint32_t cpu) {
