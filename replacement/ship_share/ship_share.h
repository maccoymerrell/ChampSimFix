#ifndef REPLACEMENT_SHIP_SHARE_H
#define REPLACEMENT_SHIP_SHARE_H
...
struct ship_share : public champsim::modules::replacement {
private:
...
public:
  ...
  std::vector<bool> prefetch_stat;

  static constexpr std::size_t epoch = 8192;
  static constexpr std::size_t global_epoch = 262144;
  static constexpr bool ONLY_AGAINST_PREFETCH = true;
  static constexpr bool CHANGE_RATE = true;
  static constexpr bool USE_MISS_RATE = true;
  static constexpr bool ENFORCE_FILL_LIMIT = true;
  static constexpr bool ENFORCE_EVICT_LIMIT = true;

  uint64_t global_epoch_counter = 0;

  //params
  std::map<uint32_t,std::size_t> DNF_MAX;
  std::map<uint32_t,std::size_t> DNF_DEFAULT;
  std::map<uint32_t,std::size_t> DNF_MIN;
  ...
  std::vector<uint64_t> do_not_fill;
  ...
  explicit ship_share(CACHE* cache);
