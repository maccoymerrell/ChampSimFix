#include "memory_system/drac_memory_system.h"
...
class DRACSystem final : public IDRACMemorySystem, public Implementation {
  RAMULATOR_REGISTER_IMPLEMENTATION(IMemorySystem, DRACSystem, "DRACSystem", "DRAC DRAM-based memory system.");

  protected:
    Clk_t m_clk = 0;
    IDRAM*  m_dram;
    IAddrMapper*  m_addr_mapper;
    std::vector<IDRACController*> m_controllers;
  ...
  public:
    void init() override { 
      ...
      for (int i = 0; i < num_channels; i++) {
        IDRACController* controller = create_child_ifce<IDRACController>();
