...
static bool was_predicted_taken;

bool tage_sc_l::predict_branch(champsim::address ip)
{
  return was_predicted_taken = predictor.GetPrediction(ip.to<uint64_t>());
}
...
void tage_sc_l::last_branch_result(champsim::address ip, champsim::address branch_target, bool taken, uint8_t branch_type)
{
  ...
  uint64_t target_val = branch_target.to<uint64_t>();
  if (branch_type == BRANCH_CONDITIONAL) {
    predictor.UpdatePredictor(ip_val, branch_type, taken, was_predicted_taken, target_val);
  } else {
    predictor.TrackOtherInst(ip_val, branch_type, taken, target_val);
  }
}
