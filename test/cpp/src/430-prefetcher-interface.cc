...
namespace
{
  ...
  struct dual_interface : champsim::modules::prefetcher
  {
    ...
    uint32_t prefetcher_cache_operate(uint64_t, uint64_t, uint32_t, uint8_t, uint32_t, uint32_t metadata_in)
    {
      ::operate_interface_discerner[intern_] = 1;
      return metadata_in;
    }

    uint32_t prefetcher_cache_operate(champsim::address, champsim::address, uint32_t, uint8_t, bool, uint32_t, uint32_t metadata_in)
    {
