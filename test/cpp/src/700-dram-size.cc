...
SCENARIO("A dram controller reports its size accurately") {
    GIVEN("A DRAM configuration of 8GB") {
        ...
        MEMORY_CONTROLLER uut{clock_period, clock_period*2, trp_cycles, trcd_cycles, tcas_cycles, tras_cycles, champsim::chrono::microseconds{64000}, {}, 64, 64, DRAM_CHANNELS, champsim::data::bytes{DRAM_CHANNEL_WIDTH}, DRAM_ROWS, DRAM_COLUMNS, DRAM_RANKS, DRAM_BANKGROUPS, DRAM_BANKS, 8192, "test/config/ramulator-yaml/ramulator_8GB.yaml"};
    ...
    GIVEN("A DRAM configuration of 16GB") {
        ...
        const std::size_t DRAM_ROWS_P_REF = 8;
        const std::size_t DRAM_CHANNEL_WIDTH = 8;
        const champsim::data::bytes expected_size{1ul << 34ul};

        MEMORY_CONTROLLER uut{clock_period, clock_period*2, trp_cycles, trcd_cycles, tcas_cycles, tras_cycles, champsim::chrono::microseconds{64000}, {}, 64, 64, DRAM_CHANNELS, champsim::data::bytes{DRAM_CHANNEL_WIDTH}, DRAM_ROWS, DRAM_COLUMNS, DRAM_RANKS, DRAM_BANKGROUPS, DRAM_BANKS, 8192,"test/config/ramulator-yaml/ramulator_16GB.yaml"};
        WHEN("The memory controller is queried for size") {
            champsim::data::bytes actual_size = uut.size();
            THEN("The memory controller reports the correct size")
            {
    ...
    GIVEN("A DRAM configuration of 32GB") {
        ...
        const std::size_t DRAM_ROWS_P_REF = 8;
        const std::size_t DRAM_CHANNEL_WIDTH = 8;
        const champsim::data::bytes expected_size{1ul << 35ul};

        MEMORY_CONTROLLER uut{clock_period, clock_period*2, trp_cycles, trcd_cycles, tcas_cycles, tras_cycles, champsim::chrono::microseconds{64000}, {}, 64, 64, DRAM_CHANNELS, champsim::data::bytes{DRAM_CHANNEL_WIDTH}, DRAM_ROWS, DRAM_COLUMNS, DRAM_RANKS, DRAM_BANKGROUPS, DRAM_BANKS, 8192, "test/config/ramulator-yaml/ramulator_32GB.yaml"};
