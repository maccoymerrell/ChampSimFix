  ...
  long operate() override
  {
    ...
    for (auto& pkt : ready_packets) {
      if (pkt.response_requested)
        queues.returned.push_back_grow(champsim::channel::response_type{pkt});
    }
    ready_packets.clear();
...
struct queue_issue_MRP : public champsim::operable {
  ...
  {
    ++cycle_count;
    auto finder = [&](response_type to_find, result_data candidate) {
      return top_finder(candidate.pkt, to_find);
    };
