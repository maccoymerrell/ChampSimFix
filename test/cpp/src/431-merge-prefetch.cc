...
struct merge_testbed {
  ...
  CACHE uut{champsim::modules::ModuleBuilder{"t431_cache", "DEFAULT_CACHE", champsim::defaults::default_l1d()}
  ...
  GIVEN("A cache with a prefetch miss")
  {
    ...
    WHEN("A " + std::string{str} + " is issued")
    {
      ...
      AND_WHEN("The MSHR is closed")
      {
        ...
        testbed.uut.lower_level->get_returned().push_back_grow(response);
