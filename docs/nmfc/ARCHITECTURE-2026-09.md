...
The tile core becomes **two independent pipelines that meet in one place**. The instruction
side: a context's instruction slot, refilled at decode from the program counter or from a
shared branch-target buffer, through instruction translation, into a banked instruction
cache. The data side: a context's load slot holding a virtual address, through
virtually-indexed translation queues, then through one delivery window, into the
physically-indexed memory queue that owns the resulting address, then its data-cache bank,
then the tile's last-level slice and its memory.
